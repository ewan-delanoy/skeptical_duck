...
if a=0 then if b=0 then (1,1,0) else (0,Private.arouez(b),abs(b)) else
if b=0 then (Private.arouez(a),0,abs(a)) else
(*now a and b are nonzero*)
let aa=abs(a) and bb=abs(b) in
if aa=bb then (Private.arouez(a),0,abs(a)) else
if aa<bb
then Private.helper_for_complete_gcd(aa,Private.arouez(a),0,bb,0,Private.arouez(b))
else Private.helper_for_complete_gcd(bb,0,Private.arouez(b),aa,Private.arouez(a),0);;
