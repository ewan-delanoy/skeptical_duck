...
let compute_factorized_term l=
   List.fold_left ( * ) 1 (Image.vorstellung (fun (p,a)->power p a) l);;

let list_of_divisors=Memoized.make(fun n->
  let (Multiset.M(temp1))=multiset_factorization n in
  let temp2=Image.vorstellung (fun (a,ea)->Ennig.doyle (fun j->(a,j)) 0 ea ) temp1 in
  let temp3=Cartesian.general_product temp2 in
  let temp4=Image.vorstellung compute_factorized_term temp3 in
