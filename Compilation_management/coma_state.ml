...
let rootless_paths_at_module cs mn=
   Image.vorstellung Dfn_full.to_rootless_line (acolytes_at_module cs mn);;
...
let needed_dirs_and_libs_in_command cmod cs mn=
  ...
  and libs=String.concat(" ")
    (Image.vorstellung(fun z->Ocaml_library.file_for_library(z)^extension)
    (needed_libs_at_module cs mn)) in
    String.concat " " ["";dirs;libs;""];;

let all_modules cs=
  Image.vorstellung (endingless_at_module cs) (ordered_list_of_modules cs);; 
...
let unregister_module_on_monitored_modules cs eless=
  ...
  let pre_desc=List.filter(
   ...
   let rootless_paths=Image.vorstellung Dfn_full.to_rootless_line acolytes in
   (cs3,rootless_paths);;     
...
let compute_subdirectories_list cs=
  let temp1=Image.vorstellung Dfa_subdirectory.without_trailing_slash (all_used_subdirs cs) in
    let temp2=Set_of_strings.sort temp1 in
    let temp3=Set_of_strings.forget_order temp2 in
    Image.vorstellung Dfa_subdirectory.of_line temp3;;
...
let find_needed_directories cs mlx ordered_ancestors=
  let temp1=Image.vorstellung (fun mn->
...
let above_one_in_several_or_inside cs l=
  let temp1=Image.vorstellung (ancestors_at_module cs) l in
...
let all_mlx_files cs=
  ...
  List.flatten(Image.vorstellung(acolytes_at_module cs) mods);;                
      
let all_mlx_paths cs=Image.vorstellung Dfn_full.to_absolute_path 
        (all_mlx_files cs);;  

let all_rootless_paths cs=
    let mods=ordered_list_of_modules cs in
    List.flatten(Image.vorstellung(rootless_paths_at_module cs) mods);;  
...
let reposition_module cs eless (l_before,l_after)=
    ...
    let indices_before=Image.vorstellung find_idx l_before
    and indices_after=Image.vorstellung find_idx l_after in
...
let update_ancs_libs_and_dirs_at_module cs mn=
  ...
  let separated_ancestors=Image.vorstellung 
  (fun nm2->
...
module PrivateThree=struct
    ...
    let message_about_circular_dependencies printer cycles= 
      ...
      let temp1=Image.vorstellung(fun cycle->
        let ttemp1=Image.vorstellung printer cycle in
    ...
    let message_about_changed_modules changed_modules=
      let temp1=Image.vorstellung Dfa_module.to_line changed_modules in
      "\n\n\n"^
    ...
    let put_md_list_back_in_order tolerate_cycles 
      ...
      let cs2=Coma_state_field.reorder cs (Image.vorstellung fst reordered_list) in    
...
let command_for_predebuggable  cs short_path=
    ...
    let nm_deps_with_subdirs = Image.vorstellung (
       fun nm->
    ...
    let libs_for_prow = 
      ...
      Ocaml_library.compute_needed_libraries_from_uncapitalized_modules_list
        (Image.vorstellung Dfa_module.to_line nm_direct_deps)) in 
    let pre_libs1=Image.vorstellung 
    ...
    let libs=String.concat(" ")
      (Image.vorstellung(fun z->Ocaml_library.file_for_library(z)^extension) pre_libs2) in 
...
let command_for_debuggable_or_executable cmod cs rootless_path=
    ...
    let cm_elements_but_the_last = Image.vorstellung (
     ...
     |triple::other_triples->
       ...
       then 
           ...
           let rejected_siblings_with_redundancies =  
              Image.vorstellung (fun (nm2,eless2,_)->(nm2,eless2) ) rejected_siblings_as_triples in 
...
let prepare_pretty_printers_for_ocamldebug cs deps = 
  let temp1 = "load_printer str.cma"::(Image.vorstellung (fun mname->
  ...
  let temp2 = Image.vorstellung (fun mname->
    let s= Dfa_module.to_line mname in 
...
let list_of_commands_for_shaft_part_of_feydeau cmod cs (opt_modulenames,opt_rootless_path)=
   ...
   let temp1=Image.vorstellung (fun mn->
    ...
    Image.vorstellung (fun cmd->(mn,endingless_at_module cs mn,cmd) ) cmds ) l in 
    List.flatten temp1;;
...
let list_of_commands_for_ternary_feydeau cmod cs short_path=
   ...
   let cmds1=Image.vorstellung (fun (_,_,cmd)->cmd) pre_cmds1
...
let end_part_of_feydeau cmod cs (opt_modnames,opt_rootless_path)=
  match cmod with 
   ...
   |_->
     ...
     let _=Image.vorstellung  Unix_command.hardcore_uc all_cmds in 
     ()
...
module Target_system_creation=struct
  ...
  module Private=struct
    ...
    let display_circular_dependencies printer l cycles= 
      ...
      let temp1=Image.vorstellung(fun cycle->
        let ttemp1=Image.vorstellung (fun j->printer (List.nth l (j-1))) cycle in
         String.concat " -> " ttemp1 
      ) cycles in
...
exception Identical_names of (((string*string) list) list);;    
      ...
      *)
      ...
      let temp2=Image.vorstellung (fun ap->
        let s=Absolute_path.to_string ap in
      ...
      then let n1=String.length s_dir in
           ...
           let tempf2=Image.vorstellung (Image.vorstellung tempf1) in
    ...
    let compute_dependencies l=
      ...
      let rec tempf=(fun (j1,(ap1,s1))->
        ...
        let ttemp2=Image.vorstellung Dfa_module.to_line ttemp1 in
        let ttempf=(fun s_nm->
          Option.filter_and_unpack (fun 
          (k,(_,s))->
        ...
        let ttemp3=Image.vorstellung ttempf ttemp2 in
        List.flatten  ttemp3
      )  in
      ...
      let table_for_coatoms=Image.vorstellung tempg temp1 in
      ...
      Image.vorstellung (fun (j,_)->snd(List.nth temp1 (j-1)) ) good_list;;
...
let delchacre_from_scratch (source_dir,dir_for_backup) cs=
  let temp1=all_mlx_paths cs in
  let temp3=temp1 in
  let temp4=Image.vorstellung (fun ap->
...
let rename_value_inside_module cs old_name new_name=
   ...
   let temp2=Image.vorstellung (endingless_at_module cs) pre_temp2 in
   let preceding_files=Image.vorstellung  (fun eless2->
...
let list_values_from_module_in_file module_name file=
   ...
   let temp3=Image.vorstellung(fun (t,(i,j))->
...
let list_values_from_module_in_modulesystem cs module_name=
   ...
   let temp3=List.flatten temp2 in
   let temp4=Image.vorstellung fst temp3 in 
...
let list_value_occurrences_in_file t file=
   ...
   Image.vorstellung (fun j->Cull_string.closeup_around_index 
      s j
   ) temp1;; 
