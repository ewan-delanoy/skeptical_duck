...
let all_used_subdirs cs =
   ...
   Image.vorstellung snd current_assoc ;;
...
let modify_all_subdirs cs f =
   ...
   let new_subdirs = Image.vorstellung (fun (key,vaal)->(key,f vaal)) old_subdirs in 
...
let modify_all_needed_dirs cs f =
   ...
   let new_needed_dirs = Image.vorstellung (fun (key,vaal)->(key,Image.vorstellung f vaal)) old_needed_dirs in 
...
let change_one_module_name wrapped_cs old_mn new_mn=
    ...
    let new_modules = Image.vorstellung (fun x->if x=old_mn then new_mn else x)(ordered_list_of_modules cs) in  
...
let restrict wrapped_cs smaller_list_of_modules =
    ...
    let among_fathers =Image.vorstellung (fun (key,fathers)->
    ...
    let new_directories = Ordered.sort Total_ordering.standard (Image.vorstellung snd new_subdirs) in 
to_t({ cs with 
...
let transplant wrapped_cs new_frontier = 
     let cs=of_t wrapped_cs in 
     let new_principal_mts=Image.vorstellung (fun (mn,_)->
     ...
     and new_mli_mts=Image.vorstellung (fun (mn,subdir)->
     ...
     and new_products_up_to_date=Image.vorstellung (fun (mn,_)->(mn,false)
     ) cs.Coma_state_t.product_up_to_date_for_module
     and new_preq_types=Image.vorstellung (fun (eless,_)->(eless,false)
     ) cs.Coma_state_t.printer_equipped_types in 
     to_t({
           cs with    
