...
let text_for_big_constants_file_in_next_world =
  ...
  let rootlesses_coming_from_modules cs needed_modules = 
          let modules_above=Image.vorstellung (fun nm->
             Coma_state.above cs 
          ...
          let collected_acolytes=List.flatten 
            (Image.vorstellung (Coma_state.acolytes_at_module cs) 
              modules_in_good_order) in 
          (modules_in_good_order,Image.vorstellung Dfn_full.to_rootless collected_acolytes);;
  ...
  let rootlesses_to_be_copied cs opt_selection =
     ...
     None -> let sr = Image.vorstellung (fun (rootless,_)->rootless) in 
             ( 
    ...
    |Some(needed_modules,imposed_subdirs)-> 
          ...
          let all_needed_modules= (Image.vorstellung Dfn_rootless.to_module compilables) 
                           @ needed_modules in 
  ...
  let commands_for_copying cs rootlesses=
     ...
     let unordered_subdirs = Image.vorstellung Dfn_rootless.to_subdirectory rootlesses in  
     let needed_subdirs = Ordered.sort Total_ordering.standard unordered_subdirs in 
     let dir_commands = Image.vorstellung (
       fun subdir ->"mkdir -p "^s_new_root^(Dfa_subdirectory.without_trailing_slash subdir)
     ) needed_subdirs 
     and file_commands = Image.vorstellung (fun rootless->
...
let cwc cs opt_selection=
    ...
    let _=Image.vorstellung Unix_command.uc 
