...
module Private = struct
  ...
  let frozen_copy cs ~destination ?(destbackupdir=default_backup_dir) ?(destgab=false) ?(destarchive=[]) summary =
      ...
      let faraway_fw1 = Fw_with_small_details.of_configuration_and_list (faraway_config,compilables@noncompilables) in  
      let faraway_fw =Fw_with_small_details.overwrite_file_if_it_exists faraway_fw1 
  ...
  let fully_developed_copy cs ~destination ?(destbackupdir=default_backup_dir) ?(destgab=false) summary=
      ...
      let restricted_cs=(if Needed_data_summary.is_everything summary
          then cs 
          else Coma_state.restrict cs modules_in_good_order
      ) in 
      let faraway_cs1 = Coma_state.transplant 
