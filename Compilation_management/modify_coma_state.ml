...
let rename_module cs old_middle_name new_nonslashed_name=
  ...
  let separated_acolytes_below=Option.filter_and_unpack(
    ...
    then Some(Image.vorstellung (Dfn_full.to_rootless) (Coma_state.acolytes_at_module cs mn))
    else None
) (Coma_state.ordered_list_of_modules cs) in
  let all_acolytes_below=List.flatten separated_acolytes_below in
  let old_acolyte_paths=Image.vorstellung Dfn_full.to_rootless old_acolytes in 
...
(!ref_for_changed_shortpaths))  in   
   ...
   let rejected_mns=Image.vorstellung snd rejected_pairs in  
   let new_preqt=Image.vorstellung(
...
let refresh cs = 
        ...
        let (cs2,rejected_pairs,_)=
          ...
          cs1 l_mod in
        let rejected_endinglesses=Image.vorstellung snd rejected_pairs in 
        let new_ptypes=Image.vorstellung (fun mn->(mn,not(List.mem mn rejected_endinglesses))) pre_preqt in 
        let new_dirs=Coma_state.compute_subdirectories_list cs2 in
...
let relocate_module_to cs mn new_subdir=
  ...
  let new_acolytes=Image.vorstellung 
  ...
  let new_preq_types=Image.vorstellung (fun (h,bowl)->
...
let rename_module cs2 old_middle_name new_nonslashed_name=
  ...
  let old_files=Image.vorstellung (fun mlx->Dfn_full.to_rootless_line mlx) old_acolytes in   
  let new_files=Image.vorstellung (fun mlx->Dfn_full.to_rootless_line mlx) 
     new_acolytes in 
  let new_eless=Dfn_full.to_endingless(List.hd new_acolytes) in
  let separated_acolytes_below=Option.filter_and_unpack(
    fun mn->
     if List.mem old_nm (Coma_state.ancestors_at_module cs2 mn)
    then Some(Image.vorstellung (Dfn_full.to_rootless) (Coma_state.acolytes_at_module cs2 mn))
    else None
) (Coma_state.ordered_list_of_modules cs2) in
  let all_acolytes_below=List.flatten separated_acolytes_below in
  let modified_files=Image.vorstellung Dfn_rootless.to_line all_acolytes_below in 
  let _=Unix_command.uc
  ...
  let replacer=Image.vorstellung(function x->if x=old_nm then new_nm else x) in
  ...
  let new_preq_types=Image.vorstellung (fun (h,bowl)->(eless_replacer h,bowl)) old_preq_types in 
...
let rename_subdirectory cs old_subdir new_subdir=
  ...
  let cs2=Coma_state_field.modify_all_needed_dirs cs1 rename_in_sd in 
   let new_dirs=Image.vorstellung rename_in_sd (Coma_state.directories cs2)
   and new_peqt=Image.vorstellung (fun (eless,is_compiled_correctly)->
       let final_eless = (
   ...
   let old_rootless_paths=Image.vorstellung (
