...
let of_string_pair (s1,s2) = 
   Concrete_object_t.Uple (Image.imagination wrap_string [s1;s2]);;
...
let of_string_triple (s1,s2,s3)=
   Concrete_object_t.Uple (Image.imagination wrap_string [s1;s2;s3]);;
...
let of_int_list l=Concrete_object_t.List (Image.imagination (fun i->Concrete_object_t.Int(i)) l);;
let to_int_list crobj = Image.imagination unwrap_int (unwrap_list crobj);;

let of_int_pair (i,j)=Concrete_object_t.Uple (Image.imagination (fun k->Concrete_object_t.Int(k)) [i;j]);;
let to_int_pair crobj = 
     let (arg1,arg2,_,_,_,_,_)=unwrap_bounded_uple crobj in
    (unwrap_int arg1,unwrap_int arg2);;

let of_string_list l=Concrete_object_t.List (Image.imagination wrap_string l);;
let to_string_list crobj = Image.imagination unwrap_string (unwrap_list crobj);;

let of_string_pair_list l= Concrete_object_t.List (Image.imagination of_string_pair l);;
let to_string_pair_list crobj = Image.imagination to_string_pair (unwrap_list crobj);;

let of_string_triple_list l= Concrete_object_t.List (Image.imagination of_string_triple l);;
let to_string_triple_list crobj = Image.imagination to_string_triple (unwrap_list crobj);;

let of_string_list_list l=Concrete_object_t.List (Image.imagination of_string_list l);;
let to_string_list_list crobj = Image.imagination to_string_list (unwrap_list crobj);;
...
let of_list of_a l= Concrete_object_t.List(Image.imagination of_a l);;
let to_list to_a crobj= Image.imagination to_a (unwrap_list crobj);;
