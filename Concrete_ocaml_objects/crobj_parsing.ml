...
let rec unparse = function 
   ...
   |Uple(l)->let temp1=Image.imagination unparse l in 
             uple_opener^(String.concat uple_separator temp1)^uple_closer
   |List(l)->let temp1=Image.imagination unparse l in 
             list_opener^(String.concat list_separator temp1)^list_closer 
   |Array(l)->let temp1=Image.imagination unparse l in 
             array_opener^(String.concat array_separator temp1)^array_closer
   |Record(l)->let temp1=Image.imagination (fun (key,vaal)->key ^ record_arrow ^ (unparse vaal))  l in 
             record_opener^(String.concat record_separator temp1)^record_closer          
   |Variant(constructor,l)->let temp1=Image.imagination unparse l in 
