...
let remove_nonnoncompilable_files fw rootless_paths =
    ...
    let removals_to_be_made = Image.imagination (
...
let relocate_compilable_files_to fw rootless_paths new_subdir=
    ...
    let displacements_to_be_made = Image.imagination (
   ...
   {
      fw with 
      Fw_wrapper_t.compilable_files = Image.imagination (fun pair->
         let (path,_)=pair in 
...
let helper_during_string_replacement fw (old_string,new_string) accu old_list=
    let new_list =Image.imagination (
       fun pair->
...
let rename_value_inside_module fw (old_name,new_name) preceding_files rootless_path=
   ...
   let new_compilable_files =Image.imagination (
       fun pair->
         let (path,_)=pair in 
...
let helper2_during_subdirectory_renaming fw (old_subdir,new_subdir) l_pairs =
     Image.imagination (helper1_during_subdirectory_renaming fw (old_subdir,new_subdir)) l_pairs;;
...
let helper2_during_inspection fw accu l_pairs =
   let new_l_pairs = Image.imagination (helper1_during_inspection fw accu) l_pairs in 
...
let rename_module_in_filename_only fw rootlesses_to_be_renamed new_module =
   ...
   let l_cmds = Image.imagination (helper1_inside_module_renaming_in_filename fw s_new_module) rootlesses_to_be_renamed in 
   let replacements = Image.imagination (helper2_inside_module_renaming_in_filename fw new_module) rootlesses_to_be_renamed  in            
...
let rename_module_in_special_files fw (old_module,new_module) =
  ...
  let new_special_files = Image.imagination (
     fun pair->
       let (rootless,mtime)=pair in 
...
let replace_value fw (preceding_files,path) (replacee,pre_replacer) =
    ...
    let s_root = Dfa_root.connectable_to_subpath (Fw_wrapper_field.root fw) in 
    let new_lines =Image.imagination (
       fun rl-> 
