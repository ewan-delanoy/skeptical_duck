...
exception Find_representatives_exn of rubber_core_list ;;

let find_representatives rcl =
    match List.assoc_opt rcl (!ref_for_representatives) with 
     Some n -> n
    |None -> raise(Find_representatives_exn rcl);;
...
let common_length rcl = List.length (List.hd (find_representatives rcl)) ;; 

let define_short_name name ll =
    let defn = Short_name ll and rcl = L name in  
    let (_,n) = Max.maximize_it (fun x->List.hd(List.rev x)) ll in 
    (
         ref_for_definitions := (defn,rcl) :: (!ref_for_definitions) ;
         ref_for_ambient_spaces := (rcl,n) :: (!ref_for_ambient_spaces) ;
         ref_for_representatives := (rcl,[List.hd ll]) :: (!ref_for_representatives) ;
    ) ;;
...
let get_representatives pt =
    match List.assoc_opt pt (!ref_for_representatives) with 
     Some answer -> answer 
     | None -> [] ;;
     
let get_enhanced_representatives carrier pt =
    match List.assoc_opt pt carrier with 
     Some priority_answer -> priority_answer 
      | None -> get_representatives pt ;;

exception Refinement_unclear of point * (int list) list ;;

let refinement_is_possible pt new_constraints =
   let reps =  get_representatives pt in 
   match Option.seek (Constraint.satisfied_by_individual new_constraints) reps with 
     Some _ -> true 
    | None -> raise (Refinement_unclear(pt,new_constraints)) ;;  
  
let common_length pt = List.length (List.hd(get_representatives pt)) ;;
...
let refinement_opt new_constraints = function 
  ...
  | Breakpoint_with_extensions(pt,old_constraints,extension) ->
    ...
    if Point.refinement_is_possible pt final_constraints 
  ...
  | Breakpoint_with_extensions(pt,old_constraints,extension) -> 
     (Point.common_length pt)+List.length extension   ;;
...
module Selector_for_hook = struct 
  ...
  let of_hook (width,breadth,n) = function   
    ...
    let old_apply_passive_repeat width b rl =
      Rubber_list.apply_new_constraints [[b;b+width;b+2*width]] rl ;; 
     
     
    let old_apply_boundary_increment width breadth n rl = 
    ...
    let old_eval selector l =  
          match selector with 
          Passive_repeat_selector(width,b) -> 
            old_apply_passive_repeat width b (List.hd l)
          | Boundary_increment_selector(width,breadth,n) ->
            old_apply_boundary_increment width breadth n (List.hd l)
           | Fork_selector ->     
            old_apply_fork l 
           | Jump_selector -> List.hd l;;


    let apply_passive_repeat pt sycom =
      let (width,b,_,_) = Point.unveil pt in 
      Sycomore_list.refinement_opt [[b;b+width;b+2*width]] sycom ;; 
           
           
    let apply_boundary_increment pt sycom = 
      let (width,breadth,n,_) = Point.unveil pt in 
      let new_constraints = Constraint.extra_constraints_from_boundary_increment width breadth n in 
      match Sycomore_list.refinement_opt new_constraints sycom with 
        None -> None 
        |Some new_sycom ->
          (
            match new_sycom with 
    ...
    let apply_fork pt = Some(Breakpoint_with_extensions(pt,[],[])) ;; 
        
    let eval pt selector l =  
      match selector with 
        Passive_repeat_selector(width,b) -> 
...
let vwzw_short_adjust old_result adjustment =
      match adjustment with 
      Leave_unchanged -> old_result 
      |Adjust extra -> Rubber_list.remove_fixed_part_on_all extra old_result;;
  
let vwzw_adjust result_opt adjustment=
    match result_opt with 
    None -> None 
    | Some old_result ->  
    Some(vwzw_short_adjust old_result adjustment) ;;  
  ...
  let example4 (* for (1,4,[]) *)= ps_list 
   ...
   let example5 (* for (1,5,[]) *)= ps_list 
    ...
    let example6 (* for (1,6,[]) *)= ps_list 
     ...
     (
      Image.image (fun z->([],[],z,1,[0])) 
     [
      [3;6;9];[3;6;8];[3;6;7];[3;5;8];[3;5;7];[3;4;7];
      [2;5;8];[2;5;7];[2;4;7];[1;4;7]
     ]) ;;  


     let example8 (* for (1,8,[]) *)= ps_list 
     [
       1,[[1]];
       2,[[1;2]];
       3,[[1;2];[1;3];[2;3]];
       ...
       7,[[1;2;4;5;7];[1;2;4;6;7];[1;3;4;6;7]];  
       8,[[1;2;4;5;7;8]];   
       9,[[1;2;4;5;7;8];[1;2;4;5;7;9];[1;2;4;5;8;9];
          [1;2;4;6;7;9];[1;2;4;6;8;9];[1;2;5;6;8;9];
          [1;3;4;6;7;9];[1;3;4;6;8;9];[1;3;5;6;8;9];
          [2;3;5;6;8;9]]
     ]
     (
      Image.image (fun z->([],[],z,1,[0])) 
     [
      [3;6;9];[3;6;8];[3;5;8];[2;5;8]
     ]) ;;  

     let example9 (* for (1,9,[]) *)= ps_list 
     [
       1,[[1]];
       2,[[1;2]];
       3,[[1;2];[1;3];[2;3]];
       ...
       9,[[1;2;4;5;7;8];[1;2;4;5;7;9];[1;2;4;5;8;9];
          ...
          [1;3;4;6;7;9];[1;3;4;6;8;9];[1;3;5;6;8;9];
          [2;3;5;6;8;9]];
       10,[[1;2;4;5;7;8;10];[1;2;4;5;7;9;10];
          [1;2;4;6;7;9;10];[1;3;4;6;7;9;10]]   
     ]
     (
      Image.image (fun z->([],[],z,1,[0])) 
     [
...
let vwzw_rose_hashtbl = Hashtbl.create 50 ;;
let vwzw_medium_hashtbl = Hashtbl.create 50 ;;
let vwzw_low_hashtbl = Hashtbl.create 50 ;;

let rose_hashtbl = Hashtbl.create 50 ;;
let medium_hashtbl = Hashtbl.create 50 ;;
let low_hashtbl = Hashtbl.create 50 ;;

let vwzw_nonhungarian_enhance_getter old_getter width breadth (n,scrappers) = 
    let z = concretize (n,scrappers) in 
    if ((width,breadth)=(1,0))||(test_for_admissiblity width breadth z) 
    then Some (Short_list[z]) 
    else 
    match Hashtbl.find_opt vwzw_rose_hashtbl (width,breadth) with 
    Some summary -> Some (Parametrized.eval_level_two summary scrappers n)
    | None ->  
    (match Hashtbl.find_opt vwzw_medium_hashtbl (width,breadth,scrappers) with 
...
let vwzw_hungarian_enhance_getter old_getter width breadth (n,scrappers) = 
  let ((width2,breadth2,scrappers2),adj) = 
    Hungarian.decompose (width,breadth,scrappers) in 
  let res_opt = vwzw_nonhungarian_enhance_getter old_getter width2 breadth2 (n,scrappers2) in  
    Hungarian.vwzw_adjust res_opt adj;;
...
let hungarian_enhance_getter old_getter pt = 
      ...
      let ((width2,breadth2,scrappers2),adj) = 
        Hungarian.decompose (width,breadth,scrappers) in 
      let res_opt = nonhungarian_enhance_getter old_getter (P(width2,breadth2,n,scrappers2)) in  
        Hungarian.adjust res_opt adj;;

let vwzw_low_getter = Hashtbl.find_opt vwzw_low_hashtbl ;;  

let low_getter = Hashtbl.find_opt low_hashtbl ;; 

let vwzw_access = vwzw_hungarian_enhance_getter vwzw_low_getter ;;   

let access = hungarian_enhance_getter low_getter ;;   


let vwzw_descendants_for_tool (width,breadth,n,scrappers) tool = 
       match tool with 
...
let vwzw_try_tool_quickly old_getter width breadth (n,scrappers) tool = 
   let nh_enhanced_getter = vwzw_nonhungarian_enhance_getter old_getter in 
   let descendants = vwzw_descendants_for_tool (width,breadth,n,scrappers) tool 
   ...
   let temp1 = Image.image (fun (uple,adj)->
      let (w,b,m,s) = uple in 
      (uple,Hungarian.vwzw_adjust (nh_enhanced_getter w b (m,s)) adj)
  ...
  ([],Selector_for_hook.old_eval selector args) ;;  
...
let try_tool_quickly old_getter pt tool = 
    ...
    let hungarian_descendants = Image.image (
       fun pt2 -> 
         let (w,b,m,s) = Point.unveil pt2 in 
         let ((w2,b2,s2),adj) =  Hungarian.decompose (w,b,s) in 
         (P(w2,b2,m,s2),adj)
     ) descendants in 
    let temp1 = Image.image (fun (pt,adj)->
       (pt,Hungarian.adjust (nh_enhanced_getter pt) adj)
   ) hungarian_descendants in   
   let (failures,successes) = List.partition (
           fun (_,opt) -> opt = None
   ) temp1 in 
   ...
   let args = Image.image (fun (_,opt)->Option.unpack opt) successes in 
   ([],Selector_for_hook.eval pt selector args) ;; 


exception Vwzw_compute_from_below_exn of int * int * int * (int list) ;;  

let vwzw_compute_from_below old_getter (width,breadth,n,scrappers) tool =
   let (missing_data,result) = 
    vwzw_try_tool_quickly old_getter width breadth (n,scrappers) tool in 
   if result = Short_list [] 
   then raise(Vwzw_compute_from_below_exn(width,breadth,n,scrappers)) 
   else  result ;; 
...
let vwzw_low_add (width,breadth,n,scrappers,tool) =
   let finder = Hashtbl.find_opt vwzw_low_hashtbl in  
   let res = vwzw_compute_from_below finder (width,breadth,n,scrappers) tool in  
   let _ = Hashtbl.replace vwzw_low_hashtbl (width,breadth,n,scrappers) res in 
   res ;;

let vwzw_med_add (width,breadth,scrappers) summary = 
  Hashtbl.replace vwzw_medium_hashtbl (width,breadth,scrappers) summary ;;

let vwzw_rose_add (width,breadth) summary = 
    Hashtbl.replace vwzw_rose_hashtbl (width,breadth) summary ;;  
...
let vwzw_find_remote_stumbling_block_or_immediate_working_tool 
       old_getter width breadth (n,scrappers) = 
   let hg_enhanced_getter = vwzw_hungarian_enhance_getter old_getter in     
   match hg_enhanced_getter width breadth (n,scrappers) with 
    Some old_answer -> ([],None) 
    | None ->
   if breadth=0 
   then let (missing_data0,result_opt0) = 
        vwzw_try_tool_quickly old_getter width breadth (n,scrappers) Jump in 
        if result_opt0<>(Short_list [])
        then ([],Some Jump)
        else (missing_data0,None)    
   else      
   let (missing_data1,result_opt1) = 
    vwzw_try_tool_quickly old_getter width breadth (n,scrappers) Passive_repeat in 
   ...
   let (missing_data2,result_opt2) = 
    vwzw_try_tool_quickly old_getter width breadth (n,scrappers) Boundary_increment in 
   ...
   let (missing_data3,result_opt3) = 
    vwzw_try_tool_quickly old_getter width breadth (n,scrappers) Fork in 
...
let find_remote_stumbling_block_or_immediate_working_tool 
  ...
  Some old_answer -> ([],None) 
  | None ->
  if Point.breadth pt=0 
  then let (missing_data0,result_opt0) = 
       try_tool_quickly old_getter pt Jump in 
       if result_opt0<>None
       then ([],Some Jump)
       else (missing_data0,None)    
  else let (missing_data1,result_opt1) = 
...
let vwzw_carrier_get carrier (width,breadth,n,scrappers) = match 
  Hashtbl.find_opt vwzw_low_hashtbl  (width,breadth,n,scrappers) with
  Some old_answer -> Some old_answer 
  | None -> List.assoc_opt  
      (width,breadth,n,scrappers) carrier ;;
     
let vwzw_carrier_add carrier (width,breadth,n,scrappers,tool) =
    let res = vwzw_compute_from_below (vwzw_carrier_get carrier) (width,breadth,n,scrappers) tool in  
...
let rec pusher_for_recursive_computation (carrier,to_be_treated)= 
    ...
    | uple :: others -> 
       ...
       vwzw_find_remote_stumbling_block_or_immediate_working_tool 
      (vwzw_carrier_get carrier) width breadth (n,scrappers) in 
      match opt_res with 
       Some tool ->
           let new_carrier = vwzw_carrier_add carrier (width,breadth,n,scrappers,tool) in 
           (new_carrier,others)
       | None -> 
         if missing_data = [] 
         then (carrier,others) 
...
let exhaust_new_line (width,breadth,scrappers) =
    ...
    let temp2 = Image.image (fun (w,b,n,s)-> 
      ...
      let (_,hook_opt) = vwzw_find_remote_stumbling_block_or_immediate_working_tool 
      (vwzw_carrier_get mutilated_carrier) w b (n,s) in 
      (n,hook_opt)
    ) temp1 in 
    ...
    let temp4 = Int_range.scale (fun n->
      (n, vwzw_hungarian_enhance_getter
      (vwzw_carrier_get carrier) width breadth (n,scrappers)))  1 30  in 
...
vwzw_rose_add (1,1) Parametrized_Example.example1 ;; 


vwzw_med_add (1,2,[]) Parametrized_Example.example2 ;; 
vwzw_med_add (1,3,[]) Parametrized_Example.example3 ;; 


vwzw_med_add (1,4,[])  Parametrized_Example.example4 ;; 
vwzw_med_add (1,5,[])  Parametrized_Example.example5 ;; 
vwzw_med_add (1,6,[])  Parametrized_Example.example6 ;; 
vwzw_med_add (1,7,[])  Parametrized_Example.example7 ;; 
vwzw_med_add (1,8,[])  Parametrized_Example.example8 ;; 
vwzw_med_add (1,9,[])  Parametrized_Example.example9 ;; 


(*    

#use "Githubbed_archive/Szemeredi_problem/currentstab_at_szemeredi_problem.ml" ;;
...
let bound = 30 ;;
for k=3 to bound do let _ =low_add(1,8,k,[],Passive_repeat) in () done ;;

let bound = 30 ;;
for k=3 to 8 do let _ =low_add(1,7,k,[],Passive_repeat) in () done ;;
low_add(1,7,9,[],Fork) ;;
for k=10 to bound do let _ =low_add(1,7,k,[],Boundary_increment) in () done ;;

let (g1,g2) = exhaust_new_line (1,10,[]);; 
let counterexamples_in_g2 = List.filter (
  fun (n,y)->y<>Parametrized.eval_ps_list Parametrized_Example.example9 n 
) g2;;


let g3 = Image.image (fun (x,Short_list y)->(x,y))  g2 ;;
let (g4,g5) = List.partition (fun (n,y)->n<11) g3 ;;
let g6 = Image.image (fun (n,y)->
  let whole = Int_range.range 1 n in 
  (n,Image.image (i_setminus whole) y)) g5;;
let g7 = snd(List.hd g6) ;;
let g8 = Image.image (fun z->([],[],z,1,[0])) g7 ;;
