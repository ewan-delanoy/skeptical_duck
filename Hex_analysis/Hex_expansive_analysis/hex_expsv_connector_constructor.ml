...
let broken_bridge (entry,p1,p2,exit) = {
    ...
    junction = Image.imagination Hex_cell.to_int_pair [ p1 ; p2 ];
    exit = exit;
    apex = None ;
    extra_active_cells = [] ;
} ;;   
...
let expand_name = function 
   Hex_expsv_inner_connector_name_t.Bridge(us)-> bridge us 
   |Broken_bridge(entry,p1,p2,exit) -> broken_bridge (entry,p1,p2,exit)
   |Typical(tic,side,is_direct) -> Hex_expsv_typical_inner_connector_name.full_constructor tic (side,is_direct) ;; 
