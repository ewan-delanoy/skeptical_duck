...
let molecular_with_actives  cells pairs =
   ...
   (Hex_molecular_linker.constructor 
      (Image.vorstellung (fun (x,y)->Hex_atomic_linker.pair(ceos x,ceos y)) pairs),
    Hex_cell_set.safe_set (Image.vorstellung ceos cells) );; 
...
let helper_for_right_eyed_claw = 
 Hex_strategy_static_constructor_t.Exhaustive_Disjunction(Image.vorstellung Hex_cell.of_string 
 ["d3";"c3";"b3"]);;
...
let ipairs_in_right_eyed_upwards_claw =
   ...
   Ordered.sort Total_ordering.standard2 (Image.vorstellung Hex_cell.to_int_pair pp);;
