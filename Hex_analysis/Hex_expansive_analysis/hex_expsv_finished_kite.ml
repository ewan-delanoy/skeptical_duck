...
let deduce_boarded_islands  untrimmed_l (birth,death) =
    ...
    let sea_entry = (fun x->(Hex_expsv_kite_springless_element.claim_sea (x)).Hex_named_connector_t.entry )
    and sea_exit = (fun x->(Hex_expsv_kite_springless_element.claim_sea (x)).Hex_named_connector_t.exit ) in 
