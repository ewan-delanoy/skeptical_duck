...
let contribution_from_island_in_active_part pk prepared_list = 
   ...
   then Hex_island.minimal_version_for_two_edged death
    ...
    Hex_cell_set.fold_merge(Image.imagination 
...
let possibly_too_large_active_part  pk =
    ...
    let prepared_l = remove_redundant_islands (Image.imagination Hex_expsv_kite_element.compress_to_springless  unprepared_l)  in 
