...
exception Claim_island_exn of Hex_expsv_kite_element_t.t ;;
exception Claim_sea_exn of Hex_expsv_kite_element_t.t ;;
exception Extract_island_exn of Hex_expsv_kite_element_t.t ;;
exception To_springless_exn of Hex_expsv_kite_element_t.t ;;

module Private = struct 


let to_readable_string = function
  Hex_expsv_kite_element_t.Earth(island)-> Hex_island.to_readable_string island
   |Sea(nc)-> Hex_expsv_nimed_connector.to_readable_string nc
   |Springboard(springboard)-> Hex_expsv_springboard.to_readable_string springboard;;

let opt_island_component elt = match elt with 
    Hex_expsv_kite_element_t.Earth(island)-> Some island
   |Sea(nc)-> None
...
let change_island_component elt new_component= match elt with 
    Hex_expsv_kite_element_t.Earth(island)-> Hex_expsv_kite_element_t.Earth(new_component)
   |Sea(nc)-> elt
   |Springboard(springboard)->
      Hex_expsv_kite_element_t.Springboard(Hex_expsv_springboard.change_island_component springboard new_component);;


let compress_to_springless elt= match elt with 
    Hex_expsv_kite_element_t.Earth(island)-> Hex_expsv_kite_springless_element_t.Earth(island)
   |Sea(nc)-> Hex_expsv_kite_springless_element_t.Sea(nc)
   |Springboard(springboard)->Hex_expsv_kite_springless_element_t.Earth(Hex_expsv_springboard.new_island springboard);;

let claim_island x = match x with 
    Hex_expsv_kite_element_t.Earth(island)-> island
   |_-> raise(Claim_island_exn(x)) ;;

let claim_sea x = match x with 
    Hex_expsv_kite_element_t.Sea(nc)-> nc
...
let of_springless = function
    Hex_expsv_kite_springless_element_t.Earth(island)-> Hex_expsv_kite_element_t.Earth(island)
   |Sea(nc)-> Hex_expsv_kite_element_t.Sea(nc);;
...
let to_molecular_linker = function
  Hex_expsv_kite_element_t.Earth(island)-> None
   |Sea(nc)-> Some(Hex_expsv_nimed_connector.to_molecular_linker nc)
