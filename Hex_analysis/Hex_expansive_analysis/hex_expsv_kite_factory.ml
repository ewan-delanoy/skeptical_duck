...
let nonsacrificial_starters eob = 
    {
      ...
      finished       = [] ;
      failures       = [] ;
      unfinished     = Hex_expsv_starters_for_kite.nonsacrificial_starters eob;
    };;    
...
let pusher factory = 
   ...
   let new_failures = List.rev_append (Image.imagination fst failures1) factory.Hex_expsv_kite_factory_t.failures in 
...
let extract_solutions l = Ordered.sort Total_ordering.standard 
   (Image.imagination (fun (_,_,_,mlclr,actv)->(mlclr,actv))  l);;
...
let sacrificial_full_solutions eob = 
   ...
   List.flatten( Image.imagination (fun (seed,pk)->
      let ttemp3 = full_solutions_from_factory (sacrificial_starter eob pk) in 
      Image.imagination (fun sol->(seed,sol)) ttemp3
   ) temp1);;  
