...
let all_translates dim nc =
   ...
   Image.imagination (add_name nm) (Hex_connector.all_translates dim cnnctr);;

let expand_all dim cnnctrs = 
    List.flatten( Image.imagination (fun cnnctr ->
...
let usual_range = Image.imagination (
   fun d -> (Hex_dimension.eleven,d)
) Hex_cardinal_direction.all;; 

let data_for_starters = 
   Image.imagination (fun (dim,d)->((dim,d),starters_for_side (dim,d)) ) usual_range;;

let data_for_middlers = 
   Image.imagination (fun dim->(dim,middlers dim) ) [Hex_dimension.eleven];;


let data_for_enders = 
   Image.imagination (fun (dim,d)->((dim,d),enders_for_side (dim,d)) ) usual_range;;
...
let inner_sea nc =
   Hex_cell_set.safe_set 
     (Image.imagination Hex_cell.of_int_pair 
...
let active_part nc =
   let inner_cells = (Image.imagination Hex_cell.of_int_pair 
...
let extra_active_cells nc =
    Hex_cell_set.safe_set 
    (Image.imagination Hex_cell.of_int_pair nc.Hex_expsv_named_connector_t.extra_active_cells);;
...
let wet_earth nc = Hex_cell_set.fold_merge
  ...
  [
     Hex_island.inner_earth nc.Hex_expsv_named_connector_t.entry ;
     Hex_island.inner_earth nc.Hex_expsv_named_connector_t.exit ;
     (Hex_cell_set.safe_set(Image.imagination Hex_cell.of_int_pair nc.Hex_expsv_named_connector_t.junction)) ;
  ] 
  ;;
