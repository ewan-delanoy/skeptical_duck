...
let place_of_death pk=
   ...
   Hex_island.get_side death_side pk.Hex_expsv_partial_kite_t.unvisited_islands ;;   
...
let test_for_finality pk = 
   ...
   match pk.Hex_expsv_partial_kite_t.steps_so_far with 
   [] -> false 
   |last_elt::_->
   match  last_elt with 
   Hex_expsv_kite_element_t.Sea(nc) -> 
      let place_of_death = place_of_death pk in 
      Hex_expsv_nimed_connector.check_exit nc place_of_death 
     ...
     (new_elt,   
     {
         pk with 
          Hex_expsv_partial_kite_t.steps_so_far = 
               (vague_new_elt::pk.Hex_expsv_partial_kite_t.steps_so_far);
          unvisited_islands = List.filter (fun x->x<>new_island ) 
             (pk.Hex_expsv_partial_kite_t.unvisited_islands);
    });;
...
let extend_with_sea pk new_nc = 
        ...
        let old_steps=pk.Hex_expsv_partial_kite_t.steps_so_far in 
        let old_seas = pk.Hex_expsv_partial_kite_t.unvisited_seas 
        and old_free_ones = pk.Hex_expsv_partial_kite_t.remaining_free_cells in 
        let selector =  List.filter 
              (fun (z,nc)->
                Hex_expsv_nimed_connector.check_disjointness new_nc nc) 
        and remaining_free_ones = Hex_cell_set.setminus old_free_ones
         (Hex_expsv_nimed_connector.inner_sea new_nc) in 
     (new_elt,   
     {
         pk with 
          Hex_expsv_partial_kite_t.steps_so_far = vague_new_elt::old_steps ;
            unvisited_seas = selector old_seas ;
            remaining_free_cells = remaining_free_ones ;
    });;

let winner pk =
   let place_of_birth = pk.Hex_expsv_partial_kite_t.place_of_birth in 
...
let last_stop pk = 
   match pk.Hex_expsv_partial_kite_t.steps_so_far with 
    [] -> Hex_expsv_kite_element_t.Earth pk.Hex_expsv_partial_kite_t.place_of_birth
   |elt::_ -> elt ;;

let constructor  first_island islands seas free_ones =
        {
            Hex_expsv_partial_kite_t.place_of_birth = first_island;
            steps_so_far =  [];
