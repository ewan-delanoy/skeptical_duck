...
let extend_with_first_alternative pk new_fa =
    ...
    and old_seas = pk.Hex_expsv_partial_kite_t.unvisited_seas
    and old_free_ones = pk.Hex_expsv_partial_kite_t.remaining_free_cells
    ...
    {
      pk with 
        Hex_expsv_partial_kite_t.unvisited_islands = restricted_islands ;
...
let extend_with_second_alternative dim pk (cell2,new_island) =
    ...
    {
      pk2 with 
        Hex_expsv_partial_kite_t.steps_so_far = 
...
let close_future_seas pk =
   let currently_added = pk.Hex_expsv_partial_kite_t.added_by_casing 
   and seas = pk.Hex_expsv_partial_kite_t.unvisited_seas  in 
...
let casings_from_one_step_advances dim pk cl_seas= 
    ...
    let close_islands = last_island :: 
         (List.flatten(Image.imagination (fun (z,nc)->
           [nc.Hex_expsv_named_connector_t.entry;nc.Hex_expsv_named_connector_t.exit]) (close_future_seas pk))) in
    let temp1 = Set_of_poly_pairs.fold_merge (Image.imagination (Hex_island.neighbors dim) close_islands)  in
...
let casings_from_seas cl_seas = Hex_cell_set.safe_set (Image.imagination fst cl_seas) ;;   
...
let light_part common_to_both = 
   Image.imagination (fun (cell,new_pk)->
...
let explore_yet_untried_path dim (cell,new_pk) =
   ...
   Image.imagination (fun (_,fst_stop,other_stops,mlclr,actv)->
        let ttemp2 = Listennou.big_tail nbr_of_common_steps (fst_stop::other_stops) in 
        Hex_expsv_first_alternative_in_springboard_t.Fa(None,cell,Image.imagination Hex_expsv_kite_element.to_springless ttemp2,mlclr,actv)
   ) temp ;;

let explore_yet_untried_paths dim paths =
   List.flatten(Image.imagination (explore_yet_untried_path dim ) paths);;
...
let heavy_part dim common_to_both =
   ...
   let one_move_solutions= Image.imagination (fun (cell,new_pk)->
...
let extensions_by_springboard_first_halves dim pk common_to_both= 
  ...
  Image.imagination (extend_with_first_alternative pk) first_halves ;;
...
let extensions_by_springboard_second_halves dim pk common_to_both fa= 
  ...
  Image.imagination (extend_with_second_alternative dim pk) second_halves ;;
...
let extensions_by_springboard_halves dim pk =
   ...
   match pk.Hex_expsv_partial_kite_t.investment with 
...
let extract_solutions l=
   ...
   let detailed_sols = Image.imagination Hex_expsv_finished_kite.solution_details  final_ones in 
