...
let light_part common_to_both = 
   Image.image (fun (cell,new_pk)->
      (cell,Hex_expsv_kate_element.extract_island (Hex_expsv_partial_kite_field.last_stop new_pk))
   ) common_to_both;;
...
let explore_yet_untried_path dim (cell,new_pk) =
   ...
   Image.image (fun (_,fst_stop,other_stops,mlclr,actv)->
        ...
        Hex_expsv_first_alternative_in_springboard_t.Fa(None,cell,Image.image Hex_expsv_kate_element.to_springless ttemp2,mlclr,actv)
   ) temp ;;
...
let extensions_by_springboard_halves dim pk =
   ...
   if Hex_expsv_kate_element.opt_island_component last_stop = None 
   then []
   else 
