...
let springless_extensions_after_island dim partial_kite last_island =
   ...
   let unchecked_islanders = Hex_expsv_nimed_connector.islanders dim last_island remaining_islands in 
   let islanders = Option.filter_and_unpack (
      fun nc-> 
      let impossibilities = Hex_cell_set.setminus (Hex_expsv_nimed_connector.inner_sea nc) 
   ...
   let selector  = Option.filter_and_unpack (
      fun (z,nc)->
        if (Hex_expsv_nimed_connector.check_entry last_island nc)
            &&(Hex_cell_set.is_included_in z abc) 
           &&(List.exists (Hex_expsv_nimed_connector.check_exit nc) remaining_islands) 
...
let springless_extensions_after_sea partial_kite last_nc =
   ...
   let compatible_islands  = List.filter (
      Hex_expsv_nimed_connector.check_exit last_nc  
...
let springless_extensions_from_last_elt dim partial_kite last_elt = match last_elt with
   ...
   | _ -> let last_island = Hex_expsv_kate_element.extract_island last_elt in 
