...
let springless_extensions_after_sea partial_kite last_nc =
   ...
   ([],Image.imagination (Hex_expsv_partial_kite_field.extend_with_island partial_kite) compatible_islands);;
...
let extensions_finished_and_non_finished dim partial_kite =
      ...
      let finished2 = Image.imagination (fun (_,pk)->Hex_expsv_finished_kite.solution_details pk) finished1 
      and unfinished2 = Image.imagination snd unfinished1 in 
...
let pusher (factory,_) = 
   ...
   let raw_result=Image.imagination (
         fun pk->
         (pk,extensions_finished_and_non_finished d pk) 
   ) uf in  
   let (failures1,nonfailures1) = List.partition (fun (_,p)->p=([],[]) ) raw_result in 
   let new_failures = List.rev_append (Image.imagination fst failures1) fa in 
   let new_moleculars = List.flatten (Image.imagination (fun (_,p)->fst p) nonfailures1)
   and new_partial_kites = List.flatten (Image.imagination (fun (_,p)->snd p) nonfailures1) in 
