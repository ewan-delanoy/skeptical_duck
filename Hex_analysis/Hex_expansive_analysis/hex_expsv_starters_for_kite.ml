...
let constructor end_of_battle islands side =
   ...
   let clean = List.filter (
      Hex_expsv_nimed_connector.check_compatiblity end_of_battle ) in 
   let pre_middle_base = clean (Hex_expsv_nimed_connector.middlers dim)
   and pre_end_base = clean (Hex_expsv_nimed_connector.enders_for_side dim side) in                             
   let first_island = Hex_island.get_side side islands  in   
   let unexpected_starters = List.filter (
       Hex_expsv_nimed_connector.check_entry first_island
   ) pre_middle_base in 
   let start_base = clean (
      (Hex_expsv_nimed_connector.starters_for_side dim side)
      @
      unexpected_starters
      @
      (Hex_expsv_nimed_connector.islanders dim first_island islands)
   ) in 
   let full_base = Ordered.sort local_cmp (Image.image (
     fun nc -> (Hex_expsv_nimed_connector.missing_earth end_of_battle nc,nc)
