...
let prepare_for_journey = 
   [
      Hex_expsv_typical_sacrifice_t.Simple_triangle, 
                ("\206\177",[(2,1)],[(1,1);(1,2)]);
      Hex_expsv_typical_sacrifice_t.Two_successive_bridges, 
...
let oppose dim (actv,pairs) = let tempf = Image.vorstellung (Hex_ipair.oppose dim) in 
   (tempf actv,tempf pairs);;

let translate (dx,dy) (actv,pairs) = let tempf = Image.vorstellung (fun (x,y)->(x+dx,y+dy)) in 
   (tempf actv,tempf pairs);;   

let reflect (actv,pairs) = let tempf = Image.vorstellung Hex_ipair.reflect  in 
   (tempf actv,tempf pairs);;   
...
let bounds (actv,pairs) = 
     let abscissas = Image.vorstellung fst actv 
     and ordinates = Image.vorstellung snd pairs in 
...
let compatible_sacrifices eob = 
   ...
   List.flatten(Image.vorstellung (
      function ((tscr,_),side) ->
        ...
        Option.filter_and_unpack (
           fun (dx,dy) -> 
             ...
             let actv = Image.vorstellung Hex_cell.of_int_pair i_actv 
             and pairs = Image.vorstellung Hex_cell.of_int_pair i_pairs  in 
...
let reconstruct_sacrificial_solutions pairs mlclr=
    ...
    let mlclr2 = Hex_molecular_linker.constructor(Image.vorstellung Hex_atomic_linker.pair temp1) in 
