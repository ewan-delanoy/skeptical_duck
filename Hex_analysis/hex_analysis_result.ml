...
let explanation_for_familiar_move other_possible_moves =
   ...
   then "familiar move"
   else let temp1 = Image.imagination Hex_cell.to_string other_possible_moves in 
...
let explanation_for_move res =
    ...
    Some(forcing_possible,expected_seq)->
        ...
        else let temp1 = Image.imagination Hex_cell.to_string expected_seq in 
...
let full_report res = 
   ...
   then ""  (* report has already been done, in Hex_analysis.deal_with_critical_accumulation  *)
   else 
