...
let of_finished_game fgame =
   ...
   let winner_ipairs = Image.vorstellung Hex_cell.to_int_pair l_winner_cells
   and loser_ipairs = Image.vorstellung Hex_cell.to_int_pair l_loser_cells in
   let associations1=Image.vorstellung (fun (i,j)->((i,j)," A ")) winner_ipairs
   and associations2=Image.vorstellung (fun (i,j)->((i,j),"EEE")) loser_ipairs in 
   {
...
let to_molecular_linker_with_active_points grid =
    ...
    let temp7=Image.vorstellung (fun (p,s) -> 
    ...
    let eyed_claws = Image.vorstellung (fun (opt,_)->Option.unpack opt) temp9 in 
    let temp2=Image.vorstellung (fun (_,((i,j),content))->
        (Hex_cell.of_int_pair (i,j),Cull_string.trim_spaces content)) temp8 in 
    let all_used_labels=Listennou.nonredundant_version(Image.vorstellung snd temp2) in 
    let temp3=Image.vorstellung (fun c0->
    ...
    let active_ones_from_eyed_claws = Image.vorstellung (
    ...
    let pairs = Image.vorstellung (fun (c,l)->
     if List.length(l)<>2
...
let read_ascii_drawing s=
  ...
  let temp2=Image.vorstellung (fun (_,line)->Cull_string.trim_spaces line) temp1 in 
  let temp3=List.filter (fun line->
    if line="" then false else (String.get line 0)<>'-'
  ) temp2 in 
  let temp4=Image.vorstellung read_row_in_drawing (Listennou.big_tail 2 temp3) in 
...
let list_for_macros=Image.vorstellung (fun (label,(tbc,side))->
...
let preprocess grid =
   ...
   let data2 = Image.vorstellung (fun (p,s)->(p,force_length_three s) ) data1 in 
   let data3 = List.filter (fun (p,s)-> (s<>"EEE") ) data2 in 
   let claws = detect_eyed_claws grid.Hex_ascii_grid_t.dimension data3 in 
   let temp1=Image.vorstellung (fun p->let ((i,j),s)=p in 
   ...
   let non_macros2=Image.vorstellung fst non_macros1 in 
   let macros2=Image.vorstellung (fun (q,opt)->let p=fst q in (p,(Option.unpack opt) p) ) macros1 in 
   ...
   let powder_from_claws =List.flatten (Image.vorstellung (fun (claw,p)->Hex_atomic_linker.ipair_support claw) claws) in   
   let unused_labels = List.filter (fun x->Set_of_polys.nmem x labels_used_by_nonmacros) list_of_default_labels in 
   let fourtuples = List.flatten(Image.vorstellung snd macros2) in
   let labeled_fourtuples = Listennou.unequal_combine_where_fst_is_smallest fourtuples unused_labels in 
   let overrider1=List.flatten(Image.vorstellung (fun (((i1,j1),(i2,j2)),s)->[((i1,j1),s);((i2,j2),s)]) labeled_fourtuples)
   and overrider2=Image.vorstellung (fun (p,_)->(fst p," A ")) (macros1) 
   and overrider3=Image.vorstellung (fun p->(p,"eee")) powder_from_claws in 
...
let of_extended_molecular (dim,winner) extmol =
   ...
   let pairs = List.flatten(Image.vorstellung (fun
      (j,(ipair1,ipair2))->
   ...
   let eyes2 = Image.vorstellung (
      fun (d1,d2,cell) -> 
         let ipair = cti cell in 
         (ipair,name_for_eyed_claw d1 d2)::
         (Image.vorstellung (fun p->(p,"eee")) (Hex_connector_data.advanced_eyed_claw d1 d2 ipair ))
   ) eyes1 in 
   let eyes = List.flatten eyes2 in 
   let actives = Image.vorstellung (fun cell->(cti cell," A ")) actv 
   and passives = Image.vorstellung (fun cell->(cti cell,"ppp")) passv in 
   {
      Hex_ascii_grid_t.beneficiary = winner ;
      dimension = dim ;
