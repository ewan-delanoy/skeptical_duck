...
let image f (Hex_cell_set_t.S x) = Image.imagination f x;;
...
let of_string enclosed_s =
   ...
   safe_set(Image.imagination Hex_cell.of_string temp1);;

let to_string (Hex_cell_set_t.S(l))=
  let temp1=Image.imagination Hex_cell.to_string l in 
  "{"^(String.concat "," temp1)^"}";;
