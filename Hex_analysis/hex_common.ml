...
let cell_list_to_string l=
  String.concat joiner (Image.imagination Hex_cell.to_string l);;
...
let all_cells (Hex_dimension_t.D dimension)=
   ...
   let temp4=Image.imagination (fun (sj,si)->Hex_cell.of_string(sj^si)) temp3 in 
...
let cell_list_of_string s =
  let temp1=Str.split (Str.regexp_string Private.joiner) s in 
    Image.imagination Hex_cell.of_string temp1;;

let cell_list_to_pretty_string l=
  let (pairs,optional_last)=Listennou.divide_by_two l in 
  let part1= String.concat " | " (Image.imagination Private.cell_pair_to_string pairs) in 
