...
let translate cnnctr (dx,dy)=  
   ...
   and (Hex_island_t.I(anchor2,elts2)) = cnnctr.Hex_connector_t.exit in 
   {
    ...
    junction = Image.imagination (fun (x,y)->(x+dx,y+dy)) (cnnctr.Hex_connector_t.junction) ;
    exit = Hex_island_t.I(anchor2,trl elts2);
    apex = Option.propagate (fun (x,y)->(x+dx,y+dy)) (cnnctr.Hex_connector_t.apex);
    extra_active_cells = Image.imagination (fun (x,y)->(x+dx,y+dy)) (cnnctr.Hex_connector_t.extra_active_cells) ;
...
let all_translates formal_dim cnnctr = 
   ...
   Image.imagination (Private.translate cnnctr) cleaned_base ;; 
...
let inner_earth cnnctr =
   Hex_cell_set.fold_merge
     [ 
        ...
        (Hex_cell_set.safe_set (Image.imagination Hex_cell.of_int_pair
...
let oppose dim cnnctr = 
  ...
  and on_pairs =  Image.imagination (Hex_ipair.oppose dim) in 
{
    ...
    extra_active_cells =  on_pairs (cnnctr.Hex_connector_t.extra_active_cells) ;
};;

let reflect cnnctr = {
    Hex_connector_t.entry = Hex_island.reflect (cnnctr.Hex_connector_t.entry);
    junction = Image.imagination Hex_ipair.reflect (cnnctr.Hex_connector_t.junction) ;
    exit = Hex_island.reflect (cnnctr.Hex_connector_t.exit);
    apex = Option.propagate Hex_ipair.reflect (cnnctr.Hex_connector_t.apex);
    extra_active_cells = Image.imagination Hex_ipair.reflect (cnnctr.Hex_connector_t.extra_active_cells) ;
};;
...
let to_default_molecular_linker cnnctr = 
    let temp1=Image.imagination Hex_cell.of_int_pair cnnctr.Hex_connector_t.junction in 
    let temp2=Listennou.extract_successive_pairs_from_even_list temp1 in 
    Hex_molecular_linker.constructor (Image.imagination Hex_atomic_linker.pair temp2);;    
