...
let empty_one = Hex_ec_double_list_tDL([],[]);;

let add_end_config ec (Hex_ec_double_list_tDL(l1,l2))=
   ...
   Hex_player_t.First_player -> 
        ...
        Hex_ec_double_list_tDL(new_l1,l2)
  |Hex_player_t.Second_player -> 
        let new_l2=Hex_end_configuration.insert_carefully ec l2 in
        Hex_ec_double_list_tDL(l1,new_l2) ;;

let absorb_move (player,cell) (Hex_ec_double_list_tDL(l1,l2))=
  match player with 
   Hex_player_t.First_player -> 
      Hex_ec_double_list_tDL
...
let immediate_dangers player (Hex_ec_double_list_tDL(l1,l2))=
  match player with 
   Hex_player_t.First_player -> 
...
let iterated_largest_unconclusive_beginning fgame (Hex_ec_double_list_tDL(l1,l2))=
...
let to_string (Hex_ec_double_list_tDL(l1,l2))=
