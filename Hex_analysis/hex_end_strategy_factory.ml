...
let compute_extmol_for_allegedly_exhaustive_disjunction factory cells indices=
   ...
   let older_extmols=Image.imagination (get_extmol_at_idx factory) indices in 
...
let create_new_strategies show_msg old_factory entries =
   ...
   let _=Image.imagination (fun (constr,comment,indices)->
...
let compute_all_end_configs 
  ...
  Hex_fles_double_list_t.DL(
      Image.imagination (fun (Hex_cog_in_machine_t.C(_,_,_,fles))->fles) l1,
      Image.imagination (fun (Hex_cog_in_machine_t.C(_,_,_,fles))->fles) l2
  );;
...
let restrict_to_strats_with_indices 
   ...
   let reindexer=Image.imagination (fun (x,y)->(y,x)) pre_reindexer in
   let new_l=Image.imagination (Hex_reindex.cog reindexer) partial_l in 
...
let remove_strats_with_indices factory  unordered_removed_indices =
   ...
   let (Hex_end_strategy_factory_t.F(dim,player,l)) = factory in 
   let old_indices=Image.imagination (
      fun (Hex_cog_in_machine_t.C(_,_,_,fles))->
...
let indices_used_in_exhaustive_disjunctions 
    ...
    let temp1 = Image.imagination (fun (Hex_cog_in_machine_t.C(constr,_,indices,_))->indices) l in 
