...
let use_move_to_simplify_one (player,cell) old_fles =
   if player = Hex_flattened_end_strategy_automatic.beneficiary  old_fles
...
let immediate_opportunities flesses =
   let temp1 = Option.filter_and_unpack (
       fun fles->
         let l=Hex_flattened_end_strategy_automatic.active_part fles in 
         if Hex_cell_set.length(l)=1 
         then let passive_set = Hex_flattened_end_strategy_automatic.passive_part fles in 
   ...
   let interesting_indices = Image.image (fun (fles,_,_)->Hex_flattened_end_strategy_automatic.index fles) temp1 in 
...
let support fles =
   ...
   [Hex_flattened_end_strategy_automatic.active_part fles;
    Hex_flattened_end_strategy_automatic.passive_part fles];;
