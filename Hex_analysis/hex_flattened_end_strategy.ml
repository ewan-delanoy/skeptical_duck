...
let use_ally_move_to_simplify_several cell old_flesses =
    Image.imagination(use_ally_move_to_simplify_one cell) old_flesses;;
...
let immediate_opportunities flesses =
   ...
   let cells = Image.imagination (fun (_,_,cell)->cell) temp1 in 
   let older_extmols = Image.imagination (fun (fles,_,_)->fles.Hex_flattened_end_strategy_t.data) temp1 in
   let interesting_indices = Image.imagination (fun (fles,_,_)->Hex_flattened_end_strategy_field.index fles) temp1 in 
