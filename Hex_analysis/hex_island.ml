...
let add_sided_cell_by_casing dim (opt_side,new_cell) l =
   ...
   let neighbors = Image.vorstellung Hex_cell.to_int_pair (Hex_cell.neighbors dim new_cell) in 
   ...
   let old_anchors = Image.vorstellung (fun (Hex_island_t.I(anchor,_))->anchor) connected in
   let new_anchor = Hex_anchor.merge old_anchors in 
   let old_earths =  Image.vorstellung (fun (Hex_island_t.I(_,z))->z) connected in 
...
let add_several_and_forget_the_adding dim added_ones old_islands =
    let ipairs = Set_of_poly_pairs.safe_set (Image.vorstellung Hex_cell.to_int_pair added_ones) in 
...
let decompose eob =     
    ...
    let pre_answer = Image.vorstellung (
       fun l->
