...
let to_readable_string (Hex_molecular_linker_t.M(l))=
   String.concat "|" (Image.imagination Hex_atomic_linker.to_readable_string l);;


end;;

let active_complement (Hex_molecular_linker_t.M(l))= 
   Hex_cell_set.fold_merge (Image.imagination Hex_atomic_linker.active_complement l);;


let constructor unordered_l= 
  let l= Ordered.sort Hex_atomic_linker.cmp unordered_l in 
  let temp1=Image.imagination (fun atl->(atl,Hex_atomic_linker.support atl)) l in 
...
let support (Hex_molecular_linker_t.M(l))= 
   Hex_cell_set.fold_merge (Image.imagination Hex_atomic_linker.support l);;
