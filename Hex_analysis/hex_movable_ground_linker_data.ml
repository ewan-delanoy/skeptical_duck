...
exception Bad_distance of Hex_movable_ground_linker_data_t.t ;;
...
let translate d_pair data =
   ...
   {
    data with  
    Hex_movable_ground_linker_data_t.apex = tr1 (data.Hex_movable_ground_linker_data_t.apex) ;
...
let force_new_apex (new_x,new_y) data =
    let (x,y) = data.Hex_movable_ground_linker_data_t.apex in 
...
let reflect_without_renormalizing data =
   ...
   {
    data with  
    Hex_movable_ground_linker_data_t.ground = Hex_cardinal_direction.reflect (data.Hex_movable_ground_linker_data_t.ground) ; 
    Hex_movable_ground_linker_data_t.apex = tr1 (data.Hex_movable_ground_linker_data_t.apex) ;
    Hex_movable_ground_linker_data_t.support = 
        (Image.image tr1 (data.Hex_movable_ground_linker_data_t.support)) ;
};; 
...
let oppose_without_renormalizing data =
   ...
   {
    data with  
    Hex_movable_ground_linker_data_t.ground = Hex_cardinal_direction.opposite (data.Hex_movable_ground_linker_data_t.ground) ; 
    Hex_movable_ground_linker_data_t.apex = tr1 (data.Hex_movable_ground_linker_data_t.apex) ;
    Hex_movable_ground_linker_data_t.support = 
        (Image.image tr1 (data.Hex_movable_ground_linker_data_t.support)) ;
};; 
...
let bs_rightwards_claw = {
    ...
    Hex_movable_ground_linker_data_t.ground = Hex_cardinal_direction_t.Right ; 
    Hex_movable_ground_linker_data_t.distance_from_ground = 3 ; 
    Hex_movable_ground_linker_data_t.is_reducible_to_pairs = true ; 
    Hex_movable_ground_linker_data_t.apex = (4,1) ;
    Hex_movable_ground_linker_data_t.support = 
...
let sb_rightwards_claw = {
    ...
    Hex_movable_ground_linker_data_t.distance_from_ground = 3 ; 
    Hex_movable_ground_linker_data_t.is_reducible_to_pairs = true ; 
    Hex_movable_ground_linker_data_t.apex = (3,1) ;
    Hex_movable_ground_linker_data_t.support = 
...
let rightwards_pyramid = {
    ...
    Hex_movable_ground_linker_data_t.ground = Hex_cardinal_direction_t.Right ; 
    Hex_movable_ground_linker_data_t.distance_from_ground = 4 ; 
    Hex_movable_ground_linker_data_t.is_reducible_to_pairs = true ; 
    Hex_movable_ground_linker_data_t.apex = (6,1) ;
    Hex_movable_ground_linker_data_t.support = 
...
let check_distance_on_finished_data formal_dim data =
    ...
    if not(Hex_cardinal_direction.test_for_distance 
         (formal_dim,data.Hex_movable_ground_linker_data_t.ground) 
          data.Hex_movable_ground_linker_data_t.apex 
            data.Hex_movable_ground_linker_data_t.distance_from_ground
           )
    then raise(Bad_distance(data))
    else 
    match Option.seek (
       fun (x,y) -> (x<1) || (x>dim) || (y<1) || (y>dim)
    ) data.Hex_movable_ground_linker_data_t.support with 
...
let cell_support data =
   Hex_cell_set.safe_set 
    (Image.image Hex_cell.of_int_pair 
      data.Hex_movable_ground_linker_data_t.support
     );;
