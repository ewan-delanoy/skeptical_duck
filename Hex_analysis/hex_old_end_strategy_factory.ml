...
let compute_parts factory (static_constructor,indices)=
   ...
   let active_parts = Image.image  (fun (Hex_old_cog_in_machine_t.C(_,_,_,ec))->ec.Hex_flattened_end_strategy_t.active_part) temp1
   and passive_parts = Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,ec))->ec.Hex_flattened_end_strategy_t.passive_part) temp1 in 
   match static_constructor with
    Hex_old_strategy_static_constructor_t.Basic_Linker(octop,active_ones,Hex_cell_pair_set_t.S(passive_pairs))->
...
let create_and_remember_already_checked_params show_msg old_factory static_constructor comment indices=
    ...
    let added_cmt=(if comment="" 
                   then (Hex_old_strategy_static_constructor.summarize_in_string static_constructor) 
                   else comment)  in 
    let sn=string_of_int(List.length(l)+1) in 
    let (fles,new_l,msg,reduncancy)=(
       match Option.seek ( 
          fun (Hex_old_cog_in_machine_t.C(static_constructor1,_,indices1,_))->
             (static_constructor1,indices1) = (static_constructor,indices)
       ) l with 
       None ->(
                presumed_fles,
                l @ [Hex_old_cog_in_machine_t.C(static_constructor,comment,indices,presumed_fles)],
                "\n\n Just created strategy number "^sn^" ("^added_cmt^" for "^(Hex_player.color player)^")\n\n",
                false
              )
      |Some(Hex_old_cog_in_machine_t.C(_,_,_,old_fles))->
...
let check_new_strategy factory static_constructor indices = match static_constructor with 
  Hex_old_strategy_static_constructor_t.Basic_Linker(octop,active_ones,passive_pairs)->check_basic_linker (octop,active_ones,passive_pairs)
...
let compute_all_end_configs (Hex_old_end_strategy_factory_t.F(_,l1),Hex_old_end_strategy_factory_t.F(_,l2))=
  Hex_fles_double_list_t.DL(
      Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,fles))->fles) l1,
      Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,fles))->fles) l2
  );;

let reconstruct_disjunction (Hex_old_end_strategy_factory_t.F(player,l)) occupied_cells indices =
   let cell_of_index=(fun k->
     let (Hex_old_cog_in_machine_t.C(_,_,_,fles)) = List.nth l (k-1) in 
...
let restrict_to_strats_with_indices (Hex_old_end_strategy_factory_t.F(player,l)) indices =
   let partial_l=List.filter (
      fun (Hex_old_cog_in_machine_t.C(_,_,_,fles))->
