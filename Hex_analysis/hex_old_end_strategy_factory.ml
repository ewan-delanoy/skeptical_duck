...
#use"Hex_analysis/hex_old_end_strategy_factory.ml";;
...
let get_elt_at_idx (Hex_old_end_strategy_factory_t.F(player,l)) k=
...
let compute_parts factory (static_constructor,indices)=
   match static_constructor with
    ...
    | Disjunction (cells)->
        ...
        let active_parts = Image.image  (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
            Hex_old_flattened_end_strategy_field.active_part fles) temp1
        and passive_parts = Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
            Hex_old_flattened_end_strategy_field.passive_part fles) temp1 in 
...
let compute_dependencies factory  (static_constructor,indices)=
   ...
   let fles = Hex_old_flattened_end_strategy_field.constructor 
...
let create_and_remember_already_checked_params show_msg old_factory static_constructor comment indices=
    ...
    let Hex_old_end_strategy_factory_t.F(player,l)=old_factory in
    ...
    let (fles,new_l,msg,reduncancy)=(
       match Option.seek ( 
          fun (Hex_old_cog_in_machine_t.C(static_constructor1,_,indices1,_,_))->
             (static_constructor1,indices1) = (static_constructor,indices)
       ) l with 
       None ->(
                presumed_fles,
                l @ [Hex_old_cog_in_machine_t.C(static_constructor,comment,indices,xtracn,presumed_fles)],
                "\n\n Just created strategy number "^sn^" ("^added_cmt^" for "^(Hex_player.color player)^")\n\n",
                false
              )
      |Some(Hex_old_cog_in_machine_t.C(_,_,_,_,old_fles))->
              let si = string_of_int(Hex_old_flattened_end_strategy_field.index old_fles) in 
              (
                 old_fles,
                 l,
    ...
    (Hex_old_end_strategy_factory_t.F(player,new_l),fles);;

let check_disjunction factory cells indices=
   let Hex_old_end_strategy_factory_t.F(player,l)=factory in 
   let temp1=Image.image (get_elt_at_idx factory) indices in 
   let active_parts  = Image.image  (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
       Hex_old_flattened_end_strategy_field.active_part fles) temp1
   and passive_parts = Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
...
let empty_one player= Hex_old_end_strategy_factory_t.F(player,[]);;

let compute_all_end_configs (Hex_old_end_strategy_factory_t.F(_,l1),Hex_old_end_strategy_factory_t.F(_,l2))=
  Hex_old_fles_double_list_t.DL(
      Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->fles) l1,
      Image.image (fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->fles) l2
  );;

let reconstruct_disjunction (Hex_old_end_strategy_factory_t.F(player,l)) occupied_cells indices =
   let cell_of_index=(fun k->
     let (Hex_old_cog_in_machine_t.C(_,_,_,_,fles)) = List.nth l (k-1) in 
     let missing_cells= Hex_cell_set.setminus 
       (Hex_old_flattened_end_strategy_field.active_part fles) occupied_cells in 
...
let of_concrete_object crobj=
    ...
    Hex_old_end_strategy_factory_t.F(Hex_player.of_concrete_object arg1, Hex_old_cog_in_machine.list_of_concrete_object arg2);;

let to_concrete_object (Hex_old_end_strategy_factory_t.F(player,l))=
   Concrete_object_t.Variant("Hex_"^"end_strategy_factory_t.F",[
      Hex_player.to_concrete_object player;
      Hex_old_cog_in_machine.list_to_concrete_object l
   ]);;
...
let restrict_to_strats_with_indices (Hex_old_end_strategy_factory_t.F(player,l)) indices =
   let partial_l=List.filter (
      fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
        List.mem (Hex_old_flattened_end_strategy_field.index fles) indices
   ) l in 
   let pre_reindexer = Ennig.index_everything indices in 
   let reindexer=Image.image (fun (x,y)->(y,x)) pre_reindexer in
   let new_l=Image.image (Hex_old_reindex.cog reindexer) partial_l in 
...
let remove_strats_with_indices factory  unordered_removed_indices =
   ...
   let old_indices=Image.image (
      fun (Hex_old_cog_in_machine_t.C(_,_,_,_,fles))->
         (Hex_old_flattened_end_strategy_field.index fles) 
   ) l in 
