...
#use"Hex_analysis/hex_old_finished_game.ml";;
...
let empty_one = {
   Hex_old_finished_game_t.dimension = 11;
...
let largest_nonsurrendering_beginning_for_one fgame fles = 
    match Hex_old_meeting_result.meet fles fgame with 
     Hex_old_meeting_result_t.Surrender(moves_before,_,_)->
       let d=List.length(fgame.Hex_old_finished_game_t.sequence_of_moves)-List.length(moves_before) 
       ...
       let new_fgame = {fgame with Hex_old_finished_game_t.sequence_of_moves = moves_before} in           
...
let extends fgame1 fgame2=
   Listennou.extends 
     fgame1.Hex_old_finished_game_t.sequence_of_moves 
     fgame2.Hex_old_finished_game_t.sequence_of_moves ;;


let simplify_by_move new_move fgame=match fgame.Hex_old_finished_game_t.sequence_of_moves with 
  []->None
  |first_move::other_moves ->
    if (first_move = new_move)&&(other_moves <> [])
    then Some({fgame with Hex_old_finished_game_t.sequence_of_moves = other_moves})
    else None;;

let first_move fgame = List.hd(fgame.Hex_old_finished_game_t.sequence_of_moves);;

let partial_unveil fgame=
  (
     fgame.Hex_old_finished_game_t.winner,
     fgame.Hex_old_finished_game_t.sequence_of_moves
  );;
...
) :> Hex_old_finished_game_t.t Total_ordering.t);;

let seek_companion_for_strategy fles fgames =
   Option.find_and_stop (
     fun fgame -> match Hex_old_meeting_result.meet fles fgame with
       Hex_old_meeting_result_t.Attack_but_no_surrender(_,_,_)->Some(fgame)
       |_->None
   ) fgames ;;

let seek_companion_for_game fgame flesses =
  Option.find_and_stop (
     fun fles -> match Hex_old_meeting_result.meet fles fgame with
       Hex_old_meeting_result_t.Attack_but_no_surrender(moves_before,_,nbr_of_moves_remaining)->
...
let of_concrete_object  crobj= 
   ...
   {
      Hex_old_finished_game_t.dimension = Concrete_object_field.unwrap_int (g dimension_label);
...
let to_concrete_object fgame =
   ...
   Concrete_object_t.Record([
     dimension_label,Concrete_object_t.Int(fgame.Hex_old_finished_game_t.dimension);
     winner_label, Hex_player.to_concrete_object(fgame.Hex_old_finished_game_t.winner);
     sequences_of_moves_label, Concrete_object_field.of_list Hex_cell.to_concrete_object(fgame.Hex_old_finished_game_t.sequence_of_moves);
   ]);;
