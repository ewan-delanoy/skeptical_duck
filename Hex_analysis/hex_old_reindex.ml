...
let flattened_end_strategy reindexer fles = 
  let old_idx = Hex_old_flattened_end_strategy_field.index fles in 
  let new_idx = local_list_assoc reindexer old_idx in 
  Hex_old_flattened_end_strategy_field.set_index fles new_idx;;


let cog reindexer (Hex_old_cog_in_machine_t.C(statconstr,msg,indices,xtracn,fles)) = 
