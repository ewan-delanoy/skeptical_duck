...
#use"Hex_analysis/hex_old_strategy_static_constructor.ml";;

*)

let summarize_in_string = function
    Hex_old_strategy_static_constructor_t.Basic_Linker(_,_,_)->"Basic linker"
   |Gluing -> "Gluing" 
...
let of_concrete_object crobj =
    let (hook,(arg1,arg2,arg3,_,_,_,_))=Concrete_object_field.unwrap_bounded_variant crobj in 
    if hook = hook_for_basic_linker 
    then Hex_old_strategy_static_constructor_t.Basic_Linker(
           Hex_octopus.of_concrete_object arg1,
           Hex_cell_set.of_concrete_object arg2,
           Hex_cell_pair_set.of_concrete_object arg3) 
    else 
    if hook = hook_for_gluing
    then Hex_old_strategy_static_constructor_t.Gluing
    else 
    if hook = hook_for_disjunction
    then Hex_old_strategy_static_constructor_t.Disjunction(
...
let to_concrete_object = function
    Hex_old_strategy_static_constructor_t.Basic_Linker(octop,cells,pairs)->
        Concrete_object_t.Variant(hook_for_basic_linker,
           [Hex_octopus.to_concrete_object octop;
            Hex_cell_set.to_concrete_object cells;
            Hex_cell_pair_set.to_concrete_object pairs])
