...
let extend_with_springboard dim pk new_sb =
    let (Hex_springboard_t.Sp(cell,path,sol1,sol2,cell2,opt_pfc)) = new_sb in 
    ...
    |Some(pfc) ->
     (match pfc with 
     Hex_possibly_final_connector_t.Final(final_nc) -> 
             Hex_springless_analysis.extend_with_final_sea pk3 final_nc 
    |Hex_possibly_final_connector_t.Nonfinal(nc) -> 
              snd(Hex_springless_analysis.extend_with_sea pk3 nc) );;
...
let explore_minimal_casings eob pk =
   ...
   (short_to_border@temp3,Image.image (fun ((cell2,pfc),_)->(cell2,Some pfc)) first_whole);;
...
let compute_springboards eob pk =
  ...
  let temp2 = Image.image (
    fun ((cell,path,sol1,sol2),(cell2,opt_pfc))->
       (cell,path,sol1,sol2,cell2,opt_pfc)
  ) temp1  in 
