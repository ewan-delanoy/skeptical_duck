...
let configs_ref = ref (Hex_ec_double_list.empty_one);;
...
let add_end_config_without_persisting ec =
   (
    configs_ref:=Hex_ec_double_list.add_end_config ec (!configs_ref);
    games_ref:=Hex_fg_double_list.take_end_config_into_account ec (!games_ref)
   );;

let add_finished_game_without_persisting fgame =
    let checked_fgame=Hex_finished_game.iterated_largest_unconclusive_beginning fgame (!configs_ref) in 
   (
