...
let to_string (Hex_strong_opening_t.O l)=
   "O("^(Hex_common.cell_list_to_string l)^")";;

let of_string text =
     let text1=Cull_string.two_sided_cutting ("O(",")") text in 
     Hex_strong_opening_t.O(Hex_common.cell_list_of_string text1);;


let simplify_by_move new_move (Hex_strong_opening_t.O l) =
  match l with 
  []->None
  |first_move::other_moves ->
    if (first_move = new_move)&&(other_moves <> [])
    then Some(Hex_strong_opening_t.O(other_moves))
    else None;;
...
) :> Hex_strong_opening_t.t Total_ordering.t);;
