...
let bounds_for_authorized_translations dim uc = match uc with 
  (Hex_unified_connector_t.Bridge(dimplyr,(cell1,cell2)))-> 
...
let oppflect dim uc = match uc with 
  (Hex_unified_connector_t.Bridge((dim,plyr),(cell1,cell2)))-> 
     Hex_unified_connector_t.Bridge((dim,Hex_player.other_player plyr),(Hex_cell_isometry.oppflect dim cell1,Hex_cell_isometry.oppflect dim cell2))    
 |Named(nc) ->Named (Hex_named_connector.oppose dim (Hex_named_connector.reflect nc));;   


let oppose dim uc = match uc with 
  (Hex_unified_connector_t.Bridge(dimplyr,(cell1,cell2)))-> 
      Hex_unified_connector_t.Bridge(dimplyr,(Hex_cell_isometry.oppose dim cell1,Hex_cell_isometry.oppose dim cell2))    
 |Named(nc) ->Named (Hex_named_connector.oppose dim nc);;  

 let opt_constructor_in_half_checked_case dimplyr (common,connectors) =
    ...
    then let l_common = Hex_cell_set.forget_order common in 
         ...
         Some(Hex_unified_connector_t.Bridge(dimplyr,(nth 1,nth 2)))
    else      
    if   (connectors <> []) 
...
let reflect uc = match uc with 
 (Hex_unified_connector_t.Bridge((dim,plyr),(cell1,cell2)))-> 
    Hex_unified_connector_t.Bridge((dim,Hex_player.other_player plyr),(Hex_cell_isometry.reflect cell1,Hex_cell_isometry.reflect cell2))    
