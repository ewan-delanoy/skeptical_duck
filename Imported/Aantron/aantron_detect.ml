...
#use"Imported/Aantron/aantron_detect.ml";;
...
(* adapted from Markup.ml/ at https://github.com/aantron/markup.ml *)
   ...
   let trim_string = Aantron_markup_common.trim_string ;;
   let is_valid_xml_char = Aantron_markup_common.is_valid_xml_char ;;
   let is_whitespace_only = Aantron_markup_common.is_whitespace_only ;;
   ...
   let iso_8859_15 =  Aantron_encoding.iso_8859_15 ;;
               ...
               if list_mem_string name names then
                 ...
                 | "content" ->
                   if charset <> None then
                     ...
                     extract_encoding (Aantron_stream_io.string value) throw begin function
   ...
   let read_xml_encoding_declaration bytes (family : Aantron_encoding.t) throw k =
     let bytes, restore = Aantron_kstream.checkpoint bytes in
     let k v = restore (); k v in
   
     let tokens =
       bytes
       |> family
       |> Aantron_input.preprocess is_valid_xml_char Aantron_markup_error.ignore_errors
       |> Aantron_xml_tokenizer.tokenize Aantron_markup_error.ignore_errors (fun _ -> None)
     in
   
     let rec prescan () =
       Aantron_kstream.next tokens throw (fun () -> k None) begin function
         | _, `Xml {Aantron_markup_common.encoding} -> k encoding
         | _, `Comment _ -> prescan ()
