...
#use"Imported/Aantron/aantron_html_parser.ml";;
...
(* This file is part of Markup.ml, released under the MIT license. See
   ...
   let html_ns = Aantron_markup_common.html_ns ;; 
   let is_scalar = Aantron_markup_common.is_whitespace ;; 
   let is_whitespace = Aantron_markup_common.is_whitespace ;; 
   let list_mem_string = Aantron_markup_common.list_mem_string ;; 
   let mathml_ns = Aantron_markup_common.mathml_ns ;; 
   let svg_ns = Aantron_markup_common.svg_ns ;; 
   let u_rep = Aantron_markup_common.u_rep ;; 

   let checkpoint = Aantron_kstream.checkpoint ;;  
   let construct = Aantron_kstream.construct ;; 
   let make = Aantron_kstream.make ;; 
   let next = Aantron_kstream.next ;; 
   let next_expected = Aantron_kstream.next_expected ;; 
   ...
   sig
     ...
     val is_not_hidden : Aantron_markup_common.Token_tag.t -> bool
   end *)
   ...
   struct
     ...
     let is_not_hidden tag =
       tag.Aantron_markup_common.Token_tag.attributes |> List.exists (fun (name, value) ->
  ...
  (* :
   ...
   module Context =
   struct
     let small_part k old_scan= function
     | _, `Doctype _ -> k `Document
     | _, `Char c when not @@ is_whitespace c -> k (`Fragment "body")
     | _, `Char _ -> old_scan ()
     | _, `EOF -> k (`Fragment "body")
     | _, `Start {Aantron_markup_common.Token_tag.name = "html"} -> k `Document
     | _, `Start {Aantron_markup_common.Token_tag.name = "head" | "body" | "frameset"} ->
       k (`Fragment "html")
     | _, `Start {Aantron_markup_common.Token_tag.name =
         "base" | "basefont" | "bgsound" | "link" | "meta" | "noframes" |
         "style" | "template" | "title"} ->
       k (`Fragment "head")
     | _, `Start {Aantron_markup_common.Token_tag.name = "frame"} -> k (`Fragment "frameset")
     | _, `Start {Aantron_markup_common.Token_tag.name = "li"} -> k (`Fragment "ul")
     | _, `Start {Aantron_markup_common.Token_tag.name =
         "caption" | "col" | "colgroup" | "tbody" | "tfoot" | "thead"} ->
       k (`Fragment "table")
     | _, `Start {Aantron_markup_common.Token_tag.name = "tr"} -> k (`Fragment "tbody")
     | _, `Start {Aantron_markup_common.Token_tag.name = "td" | "th"} -> k (`Fragment "tr")
     | _, `Start {Aantron_markup_common.Token_tag.name = "optgroup" | "option"} -> k (`Fragment "select")
     | _, `Start {Aantron_markup_common.Token_tag.name =
     ...
     let detect tokens throw k =
       ...
       let next_token k =
         next_expected tokens throw (fun token ->
           ...
           | _, `Start {Aantron_markup_common.Token_tag.name} -> last_name := Some name
           | _ -> ()
           end;
           k token)
       in
   ...
   sig
     type entry =
       | Marker
       | Element_ of element * location * Aantron_markup_common.Token_tag.t
   ...
   sig
     ...
     val adoption_agency_algorithm :
       t -> Active.t -> location -> string -> bool * (location * Aantron_markup_error.t) list
   end *)
   ...
   let parse requested_context report (tokens, set_tokenizer_state, set_foreign) =
     ...
     let report_if = Aantron_markup_error.report_if report in
     let unmatched_end_tag l name k =
       report l (`Unmatched_end_tag name) !throw k in
     let misnested_tag l t context_name k =
       report l (`Misnested_tag (t.Aantron_markup_common.Token_tag.name, context_name, t.Aantron_markup_common.Token_tag.attributes)) !throw k in
     ...
     let text = Aantron_text.prepare () in
     ...
     let add_character = Aantron_text.add text in
     ...
     and emit_text m =
       match Aantron_text.emit text with
       | None -> m ()
       | Some (l', strings) ->
     ...
     and push_and_emit
         ...
         ({Aantron_markup_common.Token_tag.name; attributes; self_closing} as tag) mode =
       ...
       let attributes =
         List.map (fun (n, v) -> Aantron_namespace.Parsing.parse n, v) attributes in
       let attributes =
         match namespace with
     ...
     and push_implicit location name mode =
       push_and_emit location
         {Aantron_markup_common.Token_tag.name = name; attributes = []; self_closing = false} mode
     ...
     and dispatch tokens rules =
       next tokens !throw (fun () -> !ended ()) begin fun ((_, t) as v) ->
         let foreign =
           ...
           | Some {element_name}, `Start {Aantron_markup_common.Token_tag.name}
           ...
           | Some {element_name = `MathML, "annotation-xml"},
               `Start {Aantron_markup_common.Token_tag.name = "svg"} -> false
     ...
     and before_html_mode () =
       dispatch tokens begin function
         ...
         | l, `Start ({Aantron_markup_common.Token_tag.name = "html"} as t) ->
           push_and_emit l t before_head_mode
   
         | l, `End {Aantron_markup_common.Token_tag.name}
     ...
     and before_head_mode () =
       dispatch tokens begin function
         ...
         | _, `Start {Aantron_markup_common.Token_tag.name = "html"} as v ->
           in_body_mode_rules "html" before_head_mode v
   
         | l, `Start ({Aantron_markup_common.Token_tag.name = "head"} as t) ->
           head_seen := true;
     ...
     and in_head_mode_rules mode = function
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name =
           "base" | "basefont" | "bgsound" | "link" | "meta"} as t) ->
         push_and_emit ~acknowledge:true l t (fun () ->
         pop l mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "title"} as t) ->
         push_and_emit l t (fun () ->
         parse_rcdata mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "noframes" | "style"} as t) ->
         push_and_emit l t (fun () ->
         parse_rawtext mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "noscript"} as t) ->
         push_and_emit l t in_head_noscript_mode
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "script"} as t) ->
         push_and_emit l t (fun () ->
         set_tokenizer_state `Script_data;
         text_mode mode)
   
       | l, `End {Aantron_markup_common.Token_tag.name = "head"} ->
         pop l after_head_mode
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "template"} as t) ->
         Active.add_marker active_formatting_elements;
         frameset_ok := false;
         Template.push template_insertion_modes in_template_mode;
         push_and_emit l t in_template_mode
   
       | l, `End {Aantron_markup_common.Token_tag.name = "template"} ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name} when not @@ list_mem_string name ["body"; "html"; "br"] ->
     ...
     and in_head_noscript_mode () =
       dispatch tokens begin function
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "noscript"} ->
           pop l in_head_mode
         ...
         | l, `Start ({Aantron_markup_common.Token_tag.name = "head" | "noscript"} as t) ->
           misnested_tag l t "noscript" in_head_noscript_mode
   
         | l, `End {Aantron_markup_common.Token_tag.name} when name <> "br" ->
     ...
     and after_head_mode () =
       dispatch tokens begin function
         ...
         | l, `Start ({Aantron_markup_common.Token_tag.name = "body"} as t) ->
           frameset_ok := false;
           push_and_emit l t in_body_mode
   
         | l, `Start ({Aantron_markup_common.Token_tag.name = "frameset"} as t) ->
         ...
         | _, `End {Aantron_markup_common.Token_tag.name = "template"} as v ->
           in_head_mode_rules after_head_mode v
   
         | l, `Start {Aantron_markup_common.Token_tag.name = "head"} ->
     ...
     and in_body_mode_rules context_name mode = function
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "body"} ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "html"} as v ->
         if not @@ Stack.in_scope open_elements "body" then
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "pre" | "listing"} as t) ->
         frameset_ok := false;
         close_current_p_element l (fun () ->
         push_and_emit l t (fun () ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "form"} as t) ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "li"} as t) ->
         frameset_ok := false;
         close_preceding_tag ["li"] l (fun () ->
         close_current_p_element l (fun () ->
         push_and_emit l t mode))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "dd" | "dt"} as t) ->
         frameset_ok := false;
         close_preceding_tag ["dd"; "dt"] l (fun () ->
         close_current_p_element l (fun () ->
         push_and_emit l t mode))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "plaintext"} as t) ->
         close_current_p_element l (fun () ->
         set_tokenizer_state `PLAINTEXT;
         push_and_emit l t mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "button"} as t) ->
         (fun mode' ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "form"} ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "p"} ->
         (fun mode' ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "li"} ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "dd" | "dt" as name} ->
         if not @@ Stack.in_scope open_elements name then
           report l (`Unmatched_end_tag name) !throw mode
         else
           close_element_with_implied name l mode
   
       | l, `End {Aantron_markup_common.Token_tag.name = "h1" | "h2" | "h3" | "h4" | "h5" | "h6" as name} ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "a"} as t) ->
         (fun k ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "nobr"} as t) ->
         Subtree.enable subtree_buffer;
         reconstruct_active_formatting_elements (fun () ->
         (fun k ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "applet" | "marquee" | "object"} as t) ->
         frameset_ok := false;
         reconstruct_active_formatting_elements (fun () ->
         Active.add_marker active_formatting_elements;
         push_and_emit l t mode)
   
       | l, `End {Aantron_markup_common.Token_tag.name = "applet" | "marquee" | "object" as name} ->
         if not @@ Stack.in_scope open_elements name then
           report l (`Unmatched_end_tag name) !throw mode
         else begin
           Active.clear_until_marker active_formatting_elements;
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "table"} as t) ->
         frameset_ok := false;
         close_current_p_element l (fun () ->
         push_and_emit l t in_table_mode)
   
       | l, `End {Aantron_markup_common.Token_tag.name = "br"} ->
         report l (`Unmatched_end_tag "br") !throw (fun () ->
         in_body_mode_rules context_name mode
           (l, `Start
             {Aantron_markup_common.Token_tag.name = "br"; attributes = []; self_closing = false}))
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "input"} as t) ->
         if Element.is_not_hidden t then frameset_ok := false;
         reconstruct_active_formatting_elements (fun () ->
         push_and_emit ~acknowledge:true l t (fun () ->
         pop l mode))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "param" | "source" | "track"} as t) ->
         push_and_emit ~acknowledge:true l t (fun () ->
         pop l mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "hr"} as t) ->
         frameset_ok := false;
         close_current_p_element l (fun () ->
         push_and_emit ~acknowledge:true l t (fun () ->
         pop l mode))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "image"} as t) ->
         report l (`Bad_token ("image", "tag", "should be 'img'")) !throw
           (fun () ->
         push tokens (l, `Start {t with name = "img"});
         mode ())
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "textarea"} as t) ->
         frameset_ok := false;
         push_and_emit l t (fun () ->
       ...
       | l, `Start {Aantron_markup_common.Token_tag.name = "xmp"} ->
         frameset_ok := false;
         close_current_p_element l (fun () ->
         reconstruct_active_formatting_elements (fun () ->
         parse_rawtext mode))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "iframe"} as t) ->
         frameset_ok := false;
         push_and_emit l t (fun () ->
         parse_rawtext mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "noembed"} as t) ->
         push_and_emit l t (fun () ->
         parse_rawtext mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "select"} as t) ->
         frameset_ok := false;
         select_in_body l t in_select_mode
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "optgroup" | "option"} as t) ->
         (fun mode' ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "rb" | "rtc"} as t) ->
         (fun mode' ->
           let finish () =
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "rp" | "rt"} as t) ->
         (fun mode' ->
           let finish () =
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "math"} as t) ->
         reconstruct_active_formatting_elements (fun () ->
         push_and_emit ~acknowledge:true ~namespace:`MathML l t (fun () ->
         if t.self_closing then pop l mode
         else mode ()))
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "svg"} as t) ->
         reconstruct_active_formatting_elements (fun () ->
     ...
     and in_table_mode_rules mode = function
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "caption"} as t) ->
         pop_to_table_context l (fun () ->
         Active.add_marker active_formatting_elements;
         push_and_emit l t in_caption_mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "colgroup"} as t) ->
         pop_to_table_context l (fun () ->
         push_and_emit l t in_column_group_mode)
   
       | l, `Start {Aantron_markup_common.Token_tag.name = "col"} as v ->
         pop_to_table_context l (fun () ->
         push tokens v;
         push_implicit l "colgroup" in_column_group_mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "tbody" | "tfoot" | "thead"} as t) ->
         pop_to_table_context l (fun () ->
         push_and_emit l t in_table_body_mode)
   
       | l, `Start {Aantron_markup_common.Token_tag.name = "td" | "th" | "tr"} as v ->
         pop_to_table_context l (fun () ->
         push tokens v;
         push_implicit l "tbody" in_table_body_mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "table"} as t) as v ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "table"} ->
       ...
       | _, `Start {Aantron_markup_common.Token_tag.name = "style" | "script" | "template"}
       | _, `End {Aantron_markup_common.Token_tag.name = "template"} as v ->
         in_head_mode_rules mode v
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "input"} as t) when Element.is_not_hidden t ->
         misnested_tag l t "table" (fun () ->
         push_and_emit ~acknowledge:true l t (fun () ->
         pop l mode))
     ...
     and in_caption_mode () =
       dispatch tokens begin function
         | l, `End {Aantron_markup_common.Token_tag.name = "caption"} ->
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "table"} as v ->
     ...
     and in_column_group_mode () =
       dispatch tokens begin function
         ...
         | l, `Start ({Aantron_markup_common.Token_tag.name = "col"} as t) ->
           push_and_emit ~acknowledge:true l t (fun () ->
           pop l in_column_group_mode)
   
         | l, `End {Aantron_markup_common.Token_tag.name = "colgroup"} ->
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "col"} ->
           report l (`Unmatched_end_tag "col") !throw in_column_group_mode
   
         | _, `Start {Aantron_markup_common.Token_tag.name = "template"}
         | _, `End {Aantron_markup_common.Token_tag.name = "template"} as v ->
     ...
     and in_table_body_mode () =
       dispatch tokens begin function
         | l, `Start ({Aantron_markup_common.Token_tag.name = "tr"} as t) ->
           pop_to_table_body_context l (fun () ->
           push_and_emit l t in_row_mode)
   
         | l, `Start ({Aantron_markup_common.Token_tag.name = "th" | "td"} as t) as v ->
           misnested_tag l t "table" (fun () ->
           pop_to_table_body_context l (fun () ->
           push tokens v;
           push_implicit l "tr" in_row_mode))
   
         | l, `End {Aantron_markup_common.Token_tag.name = "tbody" | "tfoot" | "thead" as name} ->
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "table" as name} as v ->
           if not @@ Stack.one_in_table_scope open_elements
               ["tbody"; "thead"; "tfoot"] then
             report l (`Unmatched_end_tag name) !throw in_table_body_mode
           else begin
     ...
     and in_row_mode () =
       dispatch tokens begin function
         | l, `Start ({Aantron_markup_common.Token_tag.name = "th" | "td"} as t) ->
           Active.add_marker active_formatting_elements;
           pop_to_table_row_context l (fun () ->
           push_and_emit l t in_cell_mode)
   
         | l, `End {Aantron_markup_common.Token_tag.name = "tr"} ->
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "table"} as v ->
           if not @@ Stack.in_table_scope open_elements "tr" then
             ...
             | `End {Aantron_markup_common.Token_tag.name} ->
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "tbody" | "tfoot" | "thead" as name} as v ->
           if not @@ Stack.in_table_scope open_elements name then
             report l (`Unmatched_end_tag name) !throw in_row_mode
           else
     ...
     and in_cell_mode () =
       dispatch tokens begin function
         | l, `End {Aantron_markup_common.Token_tag.name = "td" | "th" as name} ->
           if not @@ Stack.in_table_scope open_elements name then
     ...
     and in_select_mode_rules mode = function
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "option"} as t) ->
         (fun mode' ->
           if Stack.current_element_is open_elements ["option"] then pop l mode'
           else mode' ())
         (fun () -> push_and_emit l t mode)
   
       | l, `Start ({Aantron_markup_common.Token_tag.name = "optgroup"} as t) ->
         (fun mode' ->
           if Stack.current_element_is open_elements ["option"] then pop l mode'
           else mode' ())
         @@ (fun mode' () ->
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "optgroup"} ->
         (fun mode' ->
           match !open_elements with
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "option"} ->
         if Stack.current_element_is open_elements ["option"] then
           pop l mode
         else
           report l (`Unmatched_end_tag "option") !throw mode
   
       | l, `End {Aantron_markup_common.Token_tag.name = "select"} ->
       ...
       | l, `Start ({Aantron_markup_common.Token_tag.name = "input" | "keygen" | "textarea"} as t) as v ->
         misnested_tag l t "select" (fun () ->
         if not @@ Stack.in_select_scope open_elements "select" then
           mode ()
         else begin
       ...
       | _, (`Start {Aantron_markup_common.Token_tag.name = "script" | "template"} |
             `End {Aantron_markup_common.Token_tag.name = "template"}) as v ->
         in_head_mode_rules mode v
     ...
     and in_template_mode_rules mode = function
       ...
       | _, `Start {Aantron_markup_common.Token_tag.name = "col"} as v ->
         Template.pop template_insertion_modes;
         Template.push template_insertion_modes in_column_group_mode;
         push tokens v;
         in_column_group_mode ()
   
       | _, `Start {Aantron_markup_common.Token_tag.name = "tr"} as v ->
         Template.pop template_insertion_modes;
         Template.push template_insertion_modes in_table_body_mode;
         push tokens v;
         in_table_body_mode ()
   
       | _, `Start {Aantron_markup_common.Token_tag.name = "td" | "th"} as v ->
         Template.pop template_insertion_modes;
     ...
     and after_body_mode () =
       dispatch tokens begin function
         ...
         | _, `End {Aantron_markup_common.Token_tag.name = "html"} ->
     ...
     and in_frameset_mode () =
       dispatch tokens begin function
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "frameset"} ->
           (fun mode' ->
         ...
         | l, `Start ({Aantron_markup_common.Token_tag.name = "frame"} as t) ->
           push_and_emit ~acknowledge:true l t (fun () ->
           pop l in_frameset_mode)
   
         | _, `Start {Aantron_markup_common.Token_tag.name = "noframes"} as v ->
     ...
     and after_frameset_mode () =
       dispatch tokens begin function
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = "html"} ->
     ...
     and foreign_start_tag mode l tag =
       ...
       if tag.Aantron_markup_common.Token_tag.self_closing then pop l mode
       else mode ())
   
     and is_html_font_tag tag =
       tag.Aantron_markup_common.Token_tag.attributes |> List.exists (function
     ...
     and foreign_content mode force_html v =
       ...
       | l, `End {Aantron_markup_common.Token_tag.name = "script"}
           when
             match Stack.current_element open_elements with
