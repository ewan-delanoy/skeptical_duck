...
#use"Imported/Aantron/aantron_html_tokenizer.ml";;
...
(* adapted from Markup.ml/ at https://github.com/aantron/markup.ml *)
   ...
   let add_utf_8 = Aantron_markup_common.add_utf_8 ;; 
   let char = Aantron_markup_common.char ;; 
   let is_alphanumeric = Aantron_markup_common.is_alphanumeric ;;
   let is_alphabetic = Aantron_markup_common.is_alphabetic ;; 
   let is_control_character = Aantron_markup_common.is_control_character ;; 
   let is_digit = Aantron_markup_common.is_digit ;; 
   let is_hex_digit = Aantron_markup_common.is_hex_digit ;; 
   let is_non_character = Aantron_markup_common.is_non_character ;; 
   let to_lowercase = Aantron_markup_common.to_lowercase ;; 
   let is_scalar = Aantron_markup_common.is_scalar ;; 
   ...
   let next_option = Aantron_kstream.next_option ;; 
   let peek_n = Aantron_kstream.peek_n ;; 
   let peek_option = Aantron_kstream.peek_option ;; 
   let push = Aantron_kstream.push ;; 
   let push_list = Aantron_kstream.push_list ;; 
   let push_option = Aantron_kstream.push_option ;; 

   type token =
     [ `Doctype of doctype
     | `Start of Aantron_markup_common.Token_tag.t
     | `End of Aantron_markup_common.Token_tag.t
     | `Char of int
     | `Comment of string
     | `EOF ] ;;
   ...
   let named_entity_trie =
     lazy begin
       let trie = Aantron_trie.create () in
       Array.fold_left (fun trie (name, characters) ->
         Aantron_trie.add name characters trie)
         trie
         Aantron_entities.entities
     end ;;
   ...
   let tokenize report (input, get_location) =
     ...
     and emit_tag l tag' =
       ...
       let tag =
         {Aantron_markup_common.Token_tag.name = Buffer.contents tag'.tag_name;
       ...
       (fun k ->
         if tag'.start then begin
           ...
           (fun k ->
             ...
             | _ -> k ())
           @@ (fun k () ->
             if tag.Aantron_markup_common.Token_tag.self_closing then
     ...
     and emit_doctype ?(quirks = false) l doctype =
       ...
       let doctype =
         {Aantron_markup_common.doctype_name = if_not_missing doctype.doctype_name;
         ...
         | _ ->
           ...
           let rec match_named best matched replace trie text =
             next_option input !throw (function
               ...
               | Some ((_, c) as v) ->
                 let trie = Aantron_trie.advance c trie in
                 add_utf_8 text c;
                 match Aantron_trie.matches trie with
                 | Aantron_trie.No ->
                   finish best matched (v::replace)
                 | Aantron_trie.Prefix ->
                   match_named best matched (v::replace) trie text
                 | Aantron_trie.Multiple m ->
