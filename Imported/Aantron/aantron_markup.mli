...
let report =
  let count = ref 0 in
  fun location error ->
    error |>  Aantron_markup_error.to_string ~location |> prerr_endline;
    count := !count + 1;
...
(** Aantron_markup_common Internet encodings such as UTF-8 and UTF-16; also includes some less
...
sig
  ...
  val decode :
    ?report:(location ->  Aantron_markup_error.t -> unit) -> t ->
...
val parse_xml :
  ?report:(location ->  Aantron_markup_error.t -> unit) ->
  ?encoding: Aantron_encoding.t ->
...
val write_xml :
  ?report:((signal * int) ->  Aantron_markup_error.t -> unit) ->
...
(** Aantron_markup_common namespace URIs. *)
module Ns :
sig
  val html : string
...
sig
  ...
  sig
    ...
    val decode :
      ?report:(location ->  Aantron_markup_error.t -> unit io) ->  Aantron_encoding.t ->
  ...
  val parse_xml :
    ?report:(location ->  Aantron_markup_error.t -> unit io) ->
    ?encoding: Aantron_encoding.t ->
    ?namespace:(string -> string option) ->
    ?entity:(string -> string option) ->
    ?context:[< `Document | `Fragment ] ->
  ...
  val write_xml :
    ?report:((signal * int) ->  Aantron_markup_error.t -> unit io) ->
    ?prefix:(string -> string option) ->
