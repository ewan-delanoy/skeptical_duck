...
#use"Imported/Aantron/aantron_soup.ml";;

*)


(* adapted from  https://github.com/aantron/lambdasoup. *)
   ...
   and document_values =
     {mutable roots : general node list;
      doctype : Aantron_markup.doctype option}
   ...
   let from_signals' ~map_attributes signals =
     ...
     signals
     |> Aantron_markup.map (fun s ->
       begin match s with
       | `Doctype d -> doctype := Some d
       | _ -> ()
       end;
       s)
     |> (fun s -> Aantron_markup.trees
   ...
   let parse text =
     ...
     let report _l e =
       ...
       | _ -> () in
     text
     |> Aantron_markup.string
     |> (fun s -> Aantron_markup.parse_html ~report s)
     |> Aantron_markup.signals
     |> from_signals'
       ~map_attributes:(fun name attributes ->
         match name with
         | ns, "body" when ns = Aantron_markup.Ns.html ->
   ...
   *)
   ...
   let signals root =
     ...
     List.rev (traverse [] root) |> Aantron_markup.of_list ;;
   
   let pretty_print root =
     signals root
     |> Aantron_markup.pretty_print |> (fun s -> Aantron_markup.write_html s) |> Aantron_markup.to_string ;;
   
   let to_string root =
     signals root |> (fun s -> Aantron_markup.write_html s) |> Aantron_markup.to_string ;;
   ...
   let read_channel channel = Aantron_markup.channel channel |> Aantron_markup.to_string ;;
   
   let read_file path = Aantron_markup.file path |> fst |> Aantron_markup.to_string ;;
   
   let write_channel = output_string ;;
   
   let write_file path data =
     Aantron_markup.string data |> Aantron_markup.to_file path
     [@@coverage off]   ;;
