...
#use"Imported/Aantron/aantron_xml_parser.ml";;
...
(* adapted from Markup.ml/ at https://github.com/aantron/markup.ml *)
   ...
   let is_whitespace_only strings = List.for_all Aantron_markup_common.is_whitespace_only strings ;;
   
   let parse context namespace report tokens =
     let open_elements = ref [] in
     let namespaces = Aantron_namespace.Parsing.init namespace in
     ...
     and push_and_emit l {Aantron_markup_common.Token_tag.name = raw_name; attributes} state =
       Aantron_namespace.Parsing.push (fun () -> report l) namespaces raw_name attributes
     ...
     and pop l state =
       ...
       | [] -> state ()
       | _::more ->
         Aantron_namespace.Parsing.pop namespaces;
         open_elements := more;
     ...
     and root_state () =
       next_expected tokens !throw begin function
         | l, `Start t ->
           if t.Aantron_markup_common.Token_tag.self_closing then
     ...
     and content_state () =
       next_expected tokens !throw begin function
         ...
         | l, `End {Aantron_markup_common.Token_tag.name = raw_name} ->
           Aantron_namespace.Parsing.expand_element (fun () -> report l) namespaces
