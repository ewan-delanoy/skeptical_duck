...
#use"Imported/Aantron/aantron_xml_tokenizer.ml";;
...
(* adapted from Markup.ml/ at https://github.com/aantron/markup.ml *)
   ...
   (* module Token_tag = Aantron_markup_common.Token_tag ;; *)
   ...
   let is_in_range = Aantron_markup_common.is_in_range ;;
   ...
   let tokenize report resolve_reference (input, get_location) =
       ...
       and finish_xml () =
         ...
         and final_state version encoding standalone attributes =
           (fun k ->
             ...
             | [] -> k ())
           (fun () ->
             k (Some (`Xml {Aantron_markup_common.version; encoding; standalone})))
     ...
     let note_character_location = Aantron_text.note_location text in
     let add_character = Aantron_text.add text in
     let add_string = Aantron_text.add_string text in
     ...
     and emit_start l name self_closing attributes state =
       let tag = {Aantron_markup_common.Token_tag.name = name; self_closing; attributes = List.rev attributes} in
       emit l (`Start tag) state
   
     and emit_end l name state =
       let tag = {Aantron_markup_common.Token_tag.name = name; self_closing = false; attributes = []} in
       emit l (`End tag) state
   
     and emit_doctype l buffer s =
       let doctype =
         {Aantron_markup_common.doctype_name      = None;
