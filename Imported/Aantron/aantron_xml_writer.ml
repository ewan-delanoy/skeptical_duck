...
#use"Imported/Aantron/aantron_xml_writer.ml";;
...
(* adapted from Markup.ml/ at https://github.com/aantron/markup.ml *)
   ...
   let enumerate = Aantron_kstream.enumerate ;; 
  ...
  let write report prefix signals =
    ...
    let namespaces = Aantron_namespace.Writing.init prefix in
    ...
    and next_signal throw e k =
      next signals throw e begin function
        | i, (`Start_element (name, attributes) as signal) ->
          ...
          (fun self_closing ->
            Aantron_namespace.Writing.push (fun () -> report (signal, i))
            ...
            if self_closing then begin
              Aantron_namespace.Writing.pop namespaces;
              open_elements :=
        ...
        | _, `Xml {Aantron_markup_common.version; encoding; standalone} ->
          let attributes =
        ...
        | _, `Doctype {Aantron_markup_common.raw_text} ->
