...
let to_array to_a crobj= Array.map to_a (Concrete_object_field.unwrap_array crobj);;

let of_list of_a l= Concrete_object_t.List(Image.image of_a l);;
let to_list to_a crobj= Image.image to_a (Concrete_object_field.unwrap_list crobj);;
