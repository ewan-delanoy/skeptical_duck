...
module Private = struct 
  ...
  module Record = struct
    ...
    let write_converters ~tab_width rov= failwith("undefined1");;
  ...
  let write_out ~tab_width rov=
      ...
      then  Variant.write_out ~tab_width rov
      else  Record.write_out ~tab_width rov ;; 
  
  let write_converters ~tab_width rov=
      if rov.Scct_record_or_variant_t.is_variant 
      then  Variant.write_converters ~tab_width rov
      else  Variant.write_converters ~tab_width rov ;; 
