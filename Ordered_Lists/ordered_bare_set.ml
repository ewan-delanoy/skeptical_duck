...
let diforchan=((fun x->Ordered.sort cmp x):>(('a Tidel.set) list->'a set2));;
let lemel=((fun ox oy->Ordered.setminus cmp ox oy):>('a set2->'a set2->'a set2));;
let ental=((fun ox oy->Ordered.is_included_in cmp ox oy):>('a set2->'a set2->bool));;
let kengeij=((fun ox oy->Ordered.intersect cmp ox oy):>'a set2->'a set2->'a set2);;
