...
type set=Variable.t Ordered.old_set;;
let cmp=Variable.cmp;;let lt x y=Total_ordering.lt cmp x y;;
...
let min=((fun x->Ordered.min cmp x):>Variable.t list->Variable.t);;
let max=((fun x->Ordered.max cmp x):>Variable.t list->Variable.t);;
