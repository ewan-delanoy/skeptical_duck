...
module Bare = struct 
  ...
  let implode (pdf_name_start,pdf_name_end)=
      ...
      let all_pages=String.concat " " (Image.imagination snd temp4) in 
  ...
  let merge parts whole=
      let parts=Image.imagination (fun name->name^".pdf") parts in 
  ...
  let intertwine ~odd_pages ~even_pages ~num_odd ~num_even ~final_name=
    ...
    let removals=Image.imagination (fun page->"rm "^page^".pdf") pages in  
...
module Other_Tools = struct 
   ...
   let create_blank_page_with_prescribed_size (exact_width,exact_height) =
    ...
    let registered_directories = 
      Image.imagination Directory_name.of_string
      [
...
let make_booklet_naively first_arg =   
  ...
  let acts1 = Image.imagination Unix_command.uc (Command.import  old_path) in 
  ...
  let acts2 = Image.imagination Unix_command.uc 
  ...
  let acts3 = Image.imagination Unix_command.uc 
...
let append_on_the_right file1 file2 = Image.imagination Unix_command.uc 
...
let delete_file  pdfname=Image.imagination Unix_command.uc 
  (Command.delete_file  pdfname);;

let extract_page_range pdfname (i,j)=Image.imagination Unix_command.uc 
  (Command.extract_page_range pdfname (i,j));;

let extract_even_pages  pdfname=Image.imagination Unix_command.uc 
  (Command.extract_even_pages  pdfname);;

let extract_odd_pages  pdfname=Image.imagination Unix_command.uc 
...
let export ~pdfname ~new_location=Image.imagination Unix_command.uc 
  (Command.export pdfname new_location);; 

let finish_recto_verso pdfname =Image.imagination Unix_command.uc 
...
let import pdfname=Image.imagination Unix_command.uc 
  (Command.import  pdfname);;


let insert_in_just_after ~inserted_one ~receiving_one ~page_number ~initial_total_length=Image.imagination Unix_command.uc 
 (Command.insert_in_just_after inserted_one receiving_one page_number initial_total_length);;

let intertwine ~odd_pages ~even_pages ~num_odd ~num_even ~final_name=Image.imagination Unix_command.uc 
 (Command.intertwine odd_pages even_pages num_odd num_even final_name);;
  

let lay_down  pdfname=Image.imagination Unix_command.uc 
...
let prepare_recto_verso pdfname (i,j)=Image.imagination Unix_command.uc 
  (Command.prepare_recto_verso pdfname (i,j));;


let replace_page_number_in_by ~page_number ~receiving_one ~inserted_one  ~total_length=Image.imagination Unix_command.uc 
...
let upside_down  pdfname=Image.imagination Unix_command.uc 
