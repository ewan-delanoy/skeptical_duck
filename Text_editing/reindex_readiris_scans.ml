...
let indices_of_nonrenamed_files ()=
  ...
  Ordered.forget_order(Tidel.diforchan(temp2));;
...
let missing_indices_in_renamed_files ()=
   let temp1=Tidel.safe_set (indices_of_renamed_files ())
   and whole=Tidel.safe_set (Ennig.ennig 1 500) in 
   let temp2=Tidel.lemel whole temp1 in 
