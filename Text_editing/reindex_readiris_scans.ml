...
let append_pages_at_the_end ()=
  ...
  let d=i2-i1 in 
  Image.imagination (fun k->(k,k+d)) l1;;
...
let from_list=Image.imagination put_index_on_nonrenamed_file ;;
...
let append_pages_at_the_end()=Image.imagination Sys.command (Command.append_pages_at_the_end());;

let add_pages_in_between()=Image.imagination Sys.command (Command.add_pages_in_between());;
...
let g4=Image.imagination Sys.command g3;;
