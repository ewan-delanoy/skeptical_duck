...
let is_admissible (S vdw_s) = check_on_interval (S vdw_s)  1 (String.length vdw_s) ;;

let admissible_parts_are_joinable
   (S vdw_part1) (S vdw_part2) =
    let n1 = String.length vdw_part1 
    and n2 = String.length vdw_part2 in  
    check_on_interval (S(vdw_part1^vdw_part2)) (n1+1) (n1+n2) ;;

let join (S vdw_part1) (S vdw_part2) = (S(vdw_part1^vdw_part2)) ;;
