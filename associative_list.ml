...
let change_name_for_key l (key1,key2)=
   Image.vorstellung (fun pair->if fst(pair)=key1 then (key2,snd pair) else pair) l;; 

let change_value_for_key l (key1,vaal1)=
   Image.vorstellung (fun pair->if fst(pair)=key1 then (key1,vaal1) else pair) l;; 
...
let reorder l key_ordering =Image.vorstellung (fun key->(key,List.assoc key l)) key_ordering;;
...
let override_with overriden_one overrider =
  let unchecked_keys = Image.vorstellung fst (overrider@overriden_one) in 
