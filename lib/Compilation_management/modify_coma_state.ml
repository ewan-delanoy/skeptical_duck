...
*)
   ...
   module And_save = struct 
         ...
         let forget_modules fw mod_names=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2 = Fwc_final_poly.Lfwc_with_githubbing.forget_modules fw mod_names in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;
   
         let forget_nonmodular_rootlesses fw rootless_paths=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2 = Fwc_final_poly.Lfwc_with_githubbing.forget_nonmodular_rootlesses fw rootless_paths in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;
   
         let internet_access fw bowl=   
            let fw2=Fwc_final_poly.Lfwc_with_githubbing.set_gitpush_after_backup fw bowl in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;
         
         let recompile fw opt_comment=
            let fw2= Fwc_final_poly.Lfwc_with_githubbing.usual_recompile fw opt_comment in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;
         

         let refresh fw =
            let fw2= Fwc_final_poly.Lfwc_with_githubbing.of_fw_config_and_github_config 
            (Fwc_final_poly.Lfwc_with_githubbing.to_fw_configuration fw) 
            (Fwc_final_poly.Lfwc_with_githubbing.to_github_configuration fw)  in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;       

         let register_rootless_paths fw rootless_path=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2 = Fwc_final_poly.Lfwc_with_githubbing.register_rootless_paths fw rootless_path in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;  
   
         let relocate_module_to fw old_module new_subdir=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2 = Fwc_final_poly.Lfwc_with_githubbing.relocate_module_to fw old_module new_subdir in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;   
   
         let rename_module fw old_middle_name new_nonslashed_name=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2=Fwc_final_poly.Lfwc_with_githubbing.rename_module fw old_middle_name new_nonslashed_name in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;  

         let rename_subdirectory fw old_subdir new_subdir=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2=Fwc_final_poly.Lfwc_with_githubbing.rename_subdirectory_as fw (old_subdir,new_subdir) in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;  
         ...
         let replace_string fw old_s new_s=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2=Fwc_final_poly.Lfwc_with_githubbing.replace_string fw old_s new_s in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;     
         ...
         let replace_value fw ((preceding_files,path),(old_v,new_v))=
            let _=Fwc_final_poly.Lfw_with_archives.check_that_no_change_has_occurred fw in 
            let fw2= Fwc_final_poly.Lfwc_with_githubbing.replace_value fw ((preceding_files,path),(old_v,new_v)) in 
            let _=Fw_with_persisting.persist fw2 in 
            fw2;;        
   ...
   module Reference = struct 
         ...
         let initialize_if_empty pfw =
            if Fwc_final_poly.Lfw_with_dependencies.number_of_modules (!pfw) = 0 
            then initialize pfw;;
   ...
   let display_number_of_modules fw_ref =
      let number_of_modules = List.length(Fwc_final_poly.Lfw_with_dependencies.all_endinglesses (!fw_ref)) in 
   ...
   let rename_module fw_ref old_module_name new_name=
      let mn = Dfa_module.of_line(String.uncapitalize_ascii old_module_name) in
      let old_eless = Fwc_final_poly.Lfw_with_dependencies.endingless_at_module (!fw_ref) mn in
   ...
   let rename_string_or_value fw_ref old_sov new_sov =
         ...
         let endingless= Fwc_final_poly.Lfw_with_dependencies.decipher_module (!fw_ref)  module_name 
         and path= Fwc_final_poly.Lfw_with_dependencies.decipher_path (!fw_ref)  module_name in 
         let nm=Dfn_endingless.to_module endingless in
         let pre_temp2=(Fwc_final_poly.Lfw_with_dependencies.ancestors_for_module (!fw_ref) nm)@[nm] in
         let temp2=Image.image (Fwc_final_poly.Lfw_with_dependencies.endingless_at_module (!fw_ref)) pre_temp2 in
   ...
   let rename_subdirectory fw_ref old_subdirname new_subdir_short_name=
       let old_subdir = Fwc_final_poly.Lfw_with_dependencies.find_subdir_from_suffix (!fw_ref) old_subdirname  in
