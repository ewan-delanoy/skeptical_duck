...
let compute_all_needed_elesses cs needed_modules needed_subdirs =
    let all_elesses = Fw_final_poly.all_endinglesses cs in 
    ...
    let modules_above=List.flatten (Image.image (fun nm->
      Fw_final_poly.ancestors_for_module cs nm
    ) step2_modules)  in 
...
let expand fw summary =
        ...
        (match summary with 
        Needed_data_summary_t.Everything -> Fw_final_poly.all_endinglesses fw
       |Needed_data_summary_t.Selection(needed_modules,needed_subdirs)-> 
        ...
        let original_noncompilables = Fw_final_poly.noncompilable_files fw in
        (*
        ...
        *)      
        ...
        ) (Fw_final_poly.usual_compilable_files fw) in
