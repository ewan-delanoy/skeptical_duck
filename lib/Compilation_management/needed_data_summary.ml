...
let compute_all_needed_elesses cs needed_modules needed_subdirs =
    let all_elesses = Fwc_final_poly.Lfw_with_dependencies.all_endinglesses cs in 
    ...
    let modules_above=List.flatten (Image.image (fun nm->
      Fwc_final_poly.Lfw_with_dependencies.ancestors_for_module cs nm
    ) step2_modules)  in 
...
let expand fw summary =
        ...
        (match summary with 
        Needed_data_summary_t.Everything -> Fwc_final_poly.Lfw_with_dependencies.all_endinglesses fw
       |Needed_data_summary_t.Selection(needed_modules,needed_subdirs)-> 
        ...
        let original_noncompilables = Fwc_final_poly.Lfw_with_archives.noncompilable_files fw in
        (*
        ...
        *)      
        ...
        ) (Fwc_final_poly.Lfw_with_archives.usual_compilable_files fw) in
