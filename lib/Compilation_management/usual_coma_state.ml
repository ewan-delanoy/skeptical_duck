...
let main_ref=
  ...
  in 
  ref(Fw_final_poly.Lfwc_with_githubbing.plunge_fw_config_with_github_config  fw_config github_config);;
...
let all_endinglesses ()=Fw_final_poly.Lfw_with_dependencies.all_endinglesses (!(Private.main_ref)) ;; 

let changed_files_in_foreign_copy ()=
   let temp1=Fw_final_poly.Lfw_with_dependencies.all_moduled_mlx_files (!(Private.main_ref)) in 
   let this_root = Dfa_root.connectable_to_subpath (Coma_big_constant.This_World.root) 
   and next_root = Dfa_root.connectable_to_subpath (Coma_big_constant.Next_World.root) in 
...
let clean_debug_dir ()=Fw_final_poly.Lfw_with_batch_compilation.clean_debug_dir (!(Private.main_ref));;
let clean_exec_dir ()=Fw_final_poly.Lfw_with_batch_compilation.clean_exec_dir (!(Private.main_ref));;
...
Fw_final_poly.Lfw_with_dependencies.duplicate_module (!(Private.main_ref)) old_t1 old_t2;;

let find_endingless modname = 
  Fw_final_poly.Lfw_with_dependencies.endingless_at_module
...
let internet_access () = Fw_final_poly.Lfwc_with_githubbing.gitpush_after_backup (!(Private.main_ref)) ;;

let latest_changes ()= Fw_final_poly.Lfw_with_archives.latest_changes (!(Private.main_ref));;

let list_values_from_module_in_modulesystem module_name=
Fw_final_poly.Lfw_with_dependencies.list_values_from_module (!(Private.main_ref)) module_name;;

let main_ref=Private.main_ref;;

let modules_using_value x=Fw_final_poly.Lfw_with_dependencies.modules_using_value (!(Private.main_ref)) x;;
...
let show_value_occurrences_in_modulesystem module_name=
  Fw_final_poly.Lfw_with_batch_compilation.show_value_occurrences (!(Private.main_ref)) module_name;;

let start_debugging ()=Fw_final_poly.Lfw_with_batch_compilation.start_debugging (!(Private.main_ref));;
let start_executing short_path= Fw_final_poly.Lfw_with_batch_compilation.start_executing (!(Private.main_ref)) short_path;;
...
let sugared_above capitalized_or_not_module_name=
  ...
  (Fw_final_poly.Lfw_with_dependencies.ancestors_for_module (!(Private.main_ref)) mn0);;

let sugared_below capitalized_or_not_module_name=
  let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
  Image.image Dfa_module.to_line
  (Fw_final_poly.Lfw_with_dependencies.below (!(Private.main_ref)) mn0);;

let sugared_directly_above capitalized_or_not_module_name=
  let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
  Image.image Dfa_module.to_line
  (Fw_final_poly.Lfw_with_dependencies.direct_fathers_for_module (!(Private.main_ref)) mn0);;

let sugared_directly_below capitalized_or_not_module_name=
let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
Image.image Dfa_module.to_line
(Fw_final_poly.Lfw_with_dependencies.directly_below (!(Private.main_ref)) mn0);;
