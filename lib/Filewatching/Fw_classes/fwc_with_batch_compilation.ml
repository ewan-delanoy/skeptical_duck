...
module Field = struct
  ...
  module Parent = Fw_with_dependencies.Field ;;
  let parent = Fwg_with_batch_compilation.parent ;;

  let all_endinglesses fw = Fw_with_dependencies.all_endinglesses (parent fw) ;;
  let all_moduled_mlx_files fw = Fw_with_dependencies.all_moduled_mlx_files (parent fw) ;;
  let all_subdirectories fw = Fw_with_dependencies.all_subdirectories (parent fw) ;;
  let ancestors_for_module fw = Fw_with_dependencies.ancestors_for_module (parent fw) ;;
  let below fw = Fw_with_dependencies.below (parent fw) ;;
 

  let check_module_sequence_for_forgettability fw =
    Fw_with_dependencies.check_module_sequence_for_forgettability (parent fw) ;;
  ...
  let directly_below fw = Fw_with_dependencies.directly_below (parent fw) ;;
  let direct_fathers_for_module fw = Fw_with_dependencies.direct_fathers_for_module (parent fw) ;;
  let duplicate_module fw = Fw_with_dependencies.duplicate_module (parent fw) ;;
  let endingless_at_module fw = Fw_with_dependencies.endingless_at_module (parent fw) ;;
  let find_subdir_from_suffix fw = Fw_with_dependencies.find_subdir_from_suffix (parent fw) ;;
  ...
  let list_values_from_module fw =
      Fw_with_dependencies.list_values_from_module (parent fw) ;;
      
  let modules_using_value fw =
        Fw_with_dependencies.modules_using_value (parent fw) ;;    

  let  noncompilable_files fw = Parent.noncompilable_files(parent fw)  ;;  


  let number_of_modules fw =
      Fw_with_dependencies.number_of_modules (parent fw) ;;
  ...
  let show_value_occurrences fw =
    Fw_with_dependencies.show_value_occurrences (parent fw) ;;
...
let of_concrete_object ccrt_obj = 
  ...
  (Fw_with_dependencies.of_concrete_object (g label_for_parent))
...
let to_concrete_object fw = 
  let items =  
  [
       label_for_parent, Fw_with_dependencies.to_concrete_object ( Fwg_with_batch_compilation.parent fw ) ;
  ...
  let preq_types_with_extra_info fw =
      ...
      ) (Fw_with_dependencies.printer_equipped_types (parent fw)) ;;  
  ...
  let rec helper_for_feydeau  (cmod:Compilation_mode_t.t) fw (rejected,treated,to_be_treated)=
       ...
       |triple::other_triples->
         ...
         then let fw2= set_cmpl_result_at_module fw nm true in 
             ...
             (
                fun (nm2,_,_)->
                  List.mem nm (Fw_with_dependencies.ancestors_for_module (parent fw) nm2)
             ) triples_after in 
  ...
  let prepare_pretty_printers_for_ocamldebug fw deps = 
    ...
    let printable_deps = List.filter (
      fun mn -> let eless = Fw_with_dependencies.endingless_at_module (parent fw) mn in 
  ...
  let dependencies_inside_shaft cmod fw (opt_modnames,opt_rootless_path)=
     ...
     |Compilation_mode_t.Executable->let rootless_path=Option.get opt_rootless_path in 
         ...
         let deps =List.filter (fun mn->List.mem mn nm_deps) (Fw_with_dependencies.dep_ordered_modules (parent fw)) in 
  ...
  let list_of_commands_for_shaft_part_of_feydeau cmod fw (opt_modulenames,opt_rootless_path)=
     ...
     let temp1=Image.image (fun mn->
       let eless=Fw_with_dependencies.endingless_at_module pfw mn 
       and pr_ending = Fw_with_dependencies.principal_ending_for_module pfw mn in 
       let cmds=Command.module_separate_compilation cmod fw eless pr_ending in 
      Image.image (fun cmd->(mn,Fw_with_dependencies.endingless_at_module pfw mn,cmd) ) cmds ) l in 
      List.flatten temp1;;
  ...
  let up_to_date_elesses fw =
    ...
    List.filter_map (
      fun mn->
        ...
        then Some(Fw_with_dependencies.endingless_at_module pfw mn)
        else None
    )(Fw_with_dependencies.dep_ordered_modules pfw);;
  ...
  let modern_recompile fw changed_modules_in_any_order = 
   ...
   let of_fw_with_dependencies fw_with_deps = 
    ...
    (Image.image (
        fun mn -> (mn,false)
    ) (Fw_with_dependencies.dep_ordered_modules fw_with_deps));;   
  ...
  let plunge_fw_configuration config = 
    Fwg_with_batch_compilation.make (Fw_with_dependencies.plunge_fw_configuration config) [];;
