...
let to_concrete_object fw = 
 ...
 Concrete_object_t.Record items ;;
  ...
  let forget_modules fw mods = 
    ...
    let _ = Transmit_change_to_github.backup (github_configuration fw) diff (Some msg) in     
  ...
  let rename_module fw old_middle_name new_nonslashed_name = 
      ...
      let _ = Transmit_change_to_github.backup (github_configuration fw) diff2 (Some msg) in     
      set_parent ~child:fw ~new_parent ;;    
  ...
  let usual_recompile fw opt_comment = 
    ...
    let _ = Transmit_change_to_github.backup (github_configuration fw) diff opt_comment in 
    set_parent ~child:fw ~new_parent ;;

  let set_gitpush_after_backup fw gab = 
    let old_github_config = github_configuration fw in 
    let new_github_config = Fwg_github_configuration.set_gitpush_after_backup old_github_config gab in 
