...
#use"lib/Filewatching/gw_life_watcher.ml";;
...
let watched_files_field =
   {Por_field_t.field_name = "watched_files";
      field_type = "(Dfn_rootless_t.t * string) list"; 
      var_name = "files";
      default_value = "[]";
      crobj_converters =
       Some
...
Por_space.add_extension fw_space "gw_configuration" gw_life_watcher_extension;;
...
let recompute_mtime fw path =
      let s_root = Dfa_root.connectable_to_subpath (Gw_poly.root fw) 
...
let compute_changes_and_announce_them fw ~verbose=
   ...
   let (new_files,changed_files)=
       helper2_during_inspection fw ref_for_files (Gw_poly.watched_files fw) in 
   let _ = (
     if verbose 
...
let of_configuration_and_list config to_be_watched =
  let the_root = Gw_poly.root config in  
 ...
 Gw_poly.extend_gw_configuration_to_gw_life_watcher config 
...
let update_some_files fw w_files = 
   ...
   (Gw_poly.watched_files fw) in 
   Gw_poly.set_watched_files fw new_watched_files ;;
...
let apply_text_transformation_on_some_files fw tr l=
   ...
   let fw2 = Gw_poly.set_watched_files fw new_files in 
...
let first_init config =
   ...
   let (list1,_) = Unix_again.complete_ls_with_ignored_subdirs the_dir (Gw_poly.ignored_subdirectories config) false in 
   let list2 = List.filter_map(
            fun ap-> try Some(Dfn_common.decompose_absolute_path_using_root ap the_root) with 
                     _->None 
   ) list1 in
   List.filter (Gw_configuration.test_for_admissibility config) list2 ;;
...
let overwrite_file_if_it_exists fw rootless new_content =
   let root = Gw_poly.root fw in 
   if List.exists ( fun (r,_)->r=rootless ) (Gw_poly.watched_files fw)
   then let ap = Absolute_path.of_string (Dfn_common.recompose_potential_absolute_path root rootless) in 
         let _=Io.overwrite_with ap new_content in 
         let new_watched_files = update_in_list_of_pairs fw [rootless] (Gw_poly.watched_files fw) in 
         (Gw_poly.set_watched_files fw new_watched_files,true)
   else (fw,false);;


let register_rootless_paths fw rootless_paths= 
   let s_root = Dfa_root.connectable_to_subpath (Gw_poly.root fw) in
   ...
   let old_watched_files = Gw_poly.watched_files fw in    
...
let rename_files fw renaming_schemes =
  ...
  let (files,reps)   =  rename_subdirectory_on_pairs fw (old_subdir,new_subdir) 
     (Gw_poly.watched_files fw) in 
  (Gw_poly.set_watched_files fw files,reps);;   
