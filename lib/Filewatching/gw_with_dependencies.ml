...
#use"lib/Filewatching/gw_with_dependencies.ml";;
...
let index_for_caching_field =
  ...
  field_type = "Fw_instance_index_t.t * Fw_state_index_t.t";
  var_name = "cache_idx";
...
Por_space.add_extension fw_space "gw_with_small_details" gw_with_dependencies_extension;;
...
module Private = struct
 ...
 let index fw = Gw_poly.index_for_caching fw ;; 
 let parent fw = Gw_poly.parent fw ;;
 let usual_extension fw_parent instance_idx = 
    Gw_poly.extend_gw_with_small_details_to_gw_with_dependencies 
      fw_parent 
...
let forget_modules = Gw_with_small_details.forget_modules ;;

let inspect_and_update = Gw_with_small_details.inspect_and_update ;;

let of_configuration = Gw_with_small_details.of_configuration ;;

let of_configuration_and_list = Gw_with_small_details.of_configuration_and_list ;;

let overwrite_file_if_it_exists = Gw_with_small_details.overwrite_file_if_it_exists ;;

let plunge_gw_configuration = Gw_with_small_details.plunge_gw_configuration ;;

let register_rootless_paths = Gw_with_small_details.register_rootless_paths ;;

let relocate_module_to = Gw_with_small_details.relocate_module_to ;;

let remove_files = Gw_with_small_details.remove_files ;;

let rename_module_on_filename_level_and_in_files = Gw_with_small_details.rename_module_on_filename_level_and_in_files ;;

let rename_subdirectory_as = Gw_with_small_details.rename_subdirectory_as ;;

let replace_string = Gw_with_small_details.replace_string ;;

let replace_value = Gw_with_small_details.replace_value ;;end ;;
...
let plunge_gw_configuration config =  
 let new_parent = Entrance.plunge_gw_configuration config in 
 let instance_idx = Fw_indexer.create_new_instance () in 
  usual_extension new_parent instance_idx  ;; 
 ...
 let tempf = (
        fun old_pair -> 
          ...
          then let new_parent = parent new_fw in 
               (mn, Fw_module_small_details.recompute_details_for_module (Gw_with_small_details.small_details_in_files new_parent)
                    mn temp1)
          else old_pair 
      ) in 
...
let plunge_gw_configuration config =  
 let new_fw = Cached.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
 ...
 let (new_fw,extra) = visible in 
  ...
  let tempf1 = (
    fun old_pair -> 
      ...
      if temp1 <> []
      then let new_parent = parent new_fw in 
           (mn, Fw_module_small_details.recompute_details_for_module (Gw_with_small_details.small_details_in_files new_parent) mn temp1)
      else old_pair 
  ) in 
...
let plunge_gw_configuration config =  
 let new_fw = Modularized_details.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
...
let plunge_gw_configuration config =  
 let new_fw = Order.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
...
let plunge_gw_configuration config =  
 let new_fw = Needed_dirs.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
...
let plunge_gw_configuration config =  
 let new_fw = Needed_libs.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
...
let plunge_gw_configuration config =  
 let new_fw = All_subdirectories.plunge_gw_configuration config in 
 let answer = [] in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 new_fw ;;
...
) (Gw_with_archives.archived_files cs);;
...
let of_concrete_object crobj = 
    let instance_idx = Fw_indexer.create_new_instance () in   
    Gw_poly.extend_gw_with_small_details_to_gw_with_dependencies 
      (Gw_poly.of_concrete_object crobj) 
...
let plunge_gw_configuration = Private.Exit.plunge_gw_configuration ;;
...
let root fw = Gw_poly.root (Private.parent fw) ;;
...
let to_concrete_object fw = Gw_poly.to_concrete_object (Private.parent fw) ;;
let usual_compilable_files fw = Gw_with_small_details.usual_compilable_files (Private.parent fw) ;;
