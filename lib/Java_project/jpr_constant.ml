...
let location_for_data2 = 
  (Sys.getenv "HOME")^
  "/Teuliou/OCaml/Marshaled/data2.marshaled";;

let ninkasi_subdirs = 
   let john = open_in_bin location_for_data2 in 
...
let location_for_data1 = 
  (Sys.getenv "HOME")^
   "/Teuliou/OCaml/Marshaled/data1.marshaled";;

let ninkasi_package_locations = 
...
let ninkasi_package_locations = Image.image (
   fun (pkg,l) ->
    (Jpr_types.Jpkg pkg,Image.image (fun sbd ->(Jpr_types.Jsbd sbd)) l)
) Private.ninkasi_package_locations ;;  
