...
let add_new_paths_to_lr0_molecule rgy lr0_molecule paths_to_be_added =
  let (Rg old_registry_content) = rgy in 
  let (_,old_paths) = List.find (fun pair->fst(pair)=lr0_molecule) old_registry_content in 
  let new_paths = Private.path_merge old_paths (Private.path_sort paths_to_be_added) in 
  let new_registry_content = Image.image (
      fun pair -> if fst(pair)=lr0_molecule then (lr0_molecule,new_paths) else pair
  )old_registry_content in 
 Rg new_registry_content;;

let default = Rg([(St []),[]]) ;; 

let index_of_in lr0_molecule (Rg l)=
   Option.get(List.find_index  (fun pair->fst(pair)=lr0_molecule) l)-1 ;;

let register_lr0_molecule rgy lr0_molecule = 
  let (Rg old_registry_content) = rgy in 
  let (St items)=lr0_molecule in 
  match List.find_index  (fun pair->fst(pair)=lr0_molecule) old_registry_content with 
   Some old_index -> (rgy,RSt(old_index -1,St(items))) 
   | None -> 
    let new_registry_content = old_registry_content @ [lr0_molecule,[]] in 
