...
module Private = struct
  ...
  let needed_dirs_and_libs_in_command cmod fw mn=
    ...
    let s_root=Dfa_root.connectable_to_subpath(Mw_with_dependencies.root fw) in
    let dirs=
    "-I "^s_root^(Dfa_subdirectory.connectable_to_subpath(Compilation_mode.workspace cmod))
   and prelibs = Mw_with_dependencies.needed_libs_for_module fw mn in 
  ...
  let command_for_cmi (cmod:Compilation_mode_t.t) dir fw hm=
      ...
      let mli_reg=Mw_with_dependencies.check_ending_on_module fw Dfa_ocaml_ending_t.Mli  nm in
  ...
  let command_for_mli_module_separate_compilation cmod fw eless =
      let dir = Mw_with_dependencies.root fw in 
  ...
  let command_for_predebuggable fw short_path=
      let root = Mw_with_dependencies.root fw in 
      ...
      let nm_deps = Mw_with_dependencies.modules_with_their_ancestors fw nm_direct_deps in 
      let nm_deps_with_subdirs = Image.image (
         fun nm->
                 let subdir=Mw_with_dependencies.subdir_for_module fw nm in 
          (subdir,nm)
      ) nm_deps in 
      let workdir=
      ...
      let pre_libs1=Image.image 
       (fun (_,nm) -> Set_of_polys.sort(Mw_with_dependencies.needed_libs_for_module fw nm)) nm_deps_with_subdirs in
  ...
  let command_for_debuggable_or_executable cmod fw rootless_path=
    ...
    let s_root=Dfa_root.connectable_to_subpath root  in
      ...
      let nm_deps =Mw_with_dependencies.modules_with_their_ancestors fw nm_direct_deps in 
      let nm_deps_with_subdirs = Image.image (
         fun nm->let subdir=Mw_with_dependencies.subdir_for_module fw nm in 
          (subdir,nm)
      ) nm_deps in 
      let workdir=
