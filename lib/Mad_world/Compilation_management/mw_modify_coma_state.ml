...
#use"lib/Mad_world/Compilation_management/mw_modify_coma_state.ml";;
...
*)
   ...
   module And_save = struct 
         ...
         let forget_modules cs mod_names=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2 = Mw_with_githubbing.forget_modules cs mod_names in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;
   
         let forget_nonmodular_rootlesses cs rootless_paths=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2 = Mw_with_githubbing.forget_nonmodular_rootlesses cs rootless_paths in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;
   
         let internet_access cs bowl=   
            let cs2=Mw_poly.set_gitpush_after_backup cs bowl in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;
         
         let recompile cs opt_comment=
            let cs2= Mw_with_githubbing.usual_recompile cs opt_comment in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;
         

         let refresh cs =
            let cs2= Mw_with_githubbing.of_fw_config_and_github_config 
            (Mw_poly.to_fw_configuration cs) 
            (Mw_poly.to_github_configuration cs)  in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;       

         let register_rootless_paths cs rootless_path=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2 = Mw_with_githubbing.register_rootless_paths cs rootless_path in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;  
   
         let relocate_module_to cs old_module new_subdir=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2 = Mw_with_githubbing.relocate_module_to cs old_module new_subdir in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;   
   
         let rename_module cs old_middle_name new_nonslashed_name=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2=Mw_with_githubbing.rename_module cs old_middle_name new_nonslashed_name in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;  

         let rename_subdirectory cs old_subdir new_subdir=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2=Mw_with_githubbing.rename_subdirectory_as cs (old_subdir,new_subdir) in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;  
         ...
         let replace_string cs old_s new_s=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2=Mw_with_githubbing.replace_string cs old_s new_s in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;     
         ...
         let replace_value cs ((preceding_files,path),(old_v,new_v))=
            let _=Mw_with_archives.check_that_no_change_has_occurred cs in 
            let cs2= Mw_with_githubbing.replace_value cs ((preceding_files,path),(old_v,new_v)) in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;        
   ...
   module Reference = struct 
         ...
         let initialize_if_empty pcs =
            if Mw_with_dependencies.number_of_modules (!pcs) = 0 
            then initialize pcs;;
   ...
   let display_number_of_modules cs_ref =
      let number_of_modules = List.length(Mw_with_dependencies.all_endinglesses (!cs_ref)) in 
   ...
   let rename_module cs_ref old_module_name new_name=
      let mn = Dfa_module.of_line(String.uncapitalize_ascii old_module_name) in
      let old_eless = Mw_with_dependencies.endingless_at_module (!cs_ref) mn in
   ...
   let rename_string_or_value cs_ref old_sov new_sov =
         ...
         let endingless= Mw_with_dependencies.decipher_module (!cs_ref)  module_name 
         and path= Mw_with_dependencies.decipher_path (!cs_ref)  module_name in 
         let nm=Dfn_endingless.to_module endingless in
         let pre_temp2=(Mw_with_dependencies.ancestors_for_module (!cs_ref) nm)@[nm] in
         let temp2=Image.image (Mw_with_dependencies.endingless_at_module (!cs_ref)) pre_temp2 in
   ...
   let rename_subdirectory cs_ref old_subdirname new_subdir_short_name=
       let old_subdir = Mw_with_dependencies.find_subdir_from_suffix (!cs_ref) old_subdirname  in
