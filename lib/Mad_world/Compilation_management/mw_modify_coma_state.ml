...
*)
   ...
   module And_save = struct 
         ...
         let forget_modules fw mod_names=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2 = Mw_with_persisting.forget_modules fw mod_names in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;
   
         let forget_nonmodular_rootlesses cs rootless_paths=
            let _=Mw_with_persisting.check_that_no_change_has_occurred cs in 
            let cs2 = Mw_with_persisting.forget_nonmodular_rootlesses cs rootless_paths in 
            let _=Mw_with_persisting.persist cs2 in 
            cs2;;
   
         let internet_access fw bowl=   
            let fw2=Mw_with_persisting.set_gitpush_after_backup fw bowl in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;
         
         let recompile fw opt_comment=
            let fw2= Mw_with_persisting.usual_recompile fw opt_comment in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;
         ...
         let register_rootless_paths fw rootless_path=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2 = Mw_with_persisting.register_rootless_paths fw rootless_path in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;  
   
         let relocate_module_to fw old_module new_subdir=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2 = Mw_with_persisting.relocate_module_to fw old_module new_subdir in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;   
   
         let rename_module fw old_middle_name new_nonslashed_name=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2=Mw_with_persisting.rename_module fw old_middle_name new_nonslashed_name in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;  

         let rename_subdirectory fw old_subdir new_subdir=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2=Mw_with_persisting.rename_subdirectory_as fw (old_subdir,new_subdir) in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;  
         ...
         let replace_string fw old_s new_s=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2=Mw_with_persisting.replace_string fw old_s new_s in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;     
         ...
         let replace_value fw ((preceding_files,path),(old_v,new_v))=
            let _=Mw_with_persisting.check_that_no_change_has_occurred fw in 
            let fw2= Mw_with_persisting.replace_value fw ((preceding_files,path),(old_v,new_v)) in 
            let _=Mw_with_persisting.persist fw2 in 
            fw2;;        
   ...
   module Reference = struct 
         ...
         let initialize_if_empty pcs =
            if Mw_with_persisting.number_of_modules (!pcs) = 0 
            then initialize pcs;;
   ...
   let display_number_of_modules cs_ref =
      let number_of_modules = List.length(Mw_with_persisting.all_endinglesses (!cs_ref)) in 
   ...
   let rename_module cs_ref old_module_name new_name=
      let mn = Dfa_module.of_line(String.uncapitalize_ascii old_module_name) in
      let old_eless = Mw_with_persisting.endingless_at_module (!cs_ref) mn in
   ...
   let rename_string_or_value cs_ref old_sov new_sov =
         ...
         let endingless= Mw_with_persisting.decipher_module (!cs_ref)  module_name 
         and path= Mw_with_persisting.decipher_path (!cs_ref)  module_name in 
         let nm=Dfn_endingless.to_module endingless in
         let pre_temp2=(Mw_with_persisting.ancestors_for_module (!cs_ref) nm)@[nm] in
         let temp2=Image.image (Mw_with_persisting.endingless_at_module (!cs_ref)) pre_temp2 in
   ...
   let rename_subdirectory cs_ref old_subdirname new_subdir_short_name=
       let old_subdir = Mw_with_persisting.find_subdir_from_suffix (!cs_ref) old_subdirname  in
