...
#use"lib/Mad_world/Compilation_management/mw_needed_data_summary.ml";;
...
let compute_all_needed_elesses cs needed_modules needed_subdirs =
    let all_elesses = Mw_with_dependencies.all_endinglesses cs in 
    ...
    let modules_above=List.flatten (Image.image (fun nm->
       Mw_with_dependencies.ancestors_for_module cs nm
    ) step2_modules)  in 
...
let expand fw summary =
       ...
       |Mw_needed_data_summary_t.Selection(needed_modules,needed_subdirs)-> 
        ...
        let original_noncompilables = Mw_with_archives.noncompilable_files fw in
        (*
        ...
        *)      
        ...
        ) (Mw_with_archives.usual_compilable_files fw) in
