...
let all_endinglesses ()=Mw_with_dependencies.all_endinglesses (!(Private.main_ref)) ;; 

let clean_debug_dir ()=Mw_with_batch_compilation.clean_debug_dir (!(Private.main_ref));;
let clean_exec_dir ()=Mw_with_batch_compilation.clean_exec_dir (!(Private.main_ref));;
...
let internet_access () = Mw_poly.gitpush_after_backup (!(Private.main_ref)) ;;

let latest_changes ()=Mw_with_archives.latest_changes (!(Private.main_ref));;
...
let modules_using_value x=Mw_with_dependencies.modules_using_value (!(Private.main_ref)) x;;
...
let show_value_occurrences_in_modulesystem module_name=
   Mw_with_batch_compilation.show_value_occurrences (!(Private.main_ref)) module_name;;

let start_debugging ()=Mw_with_batch_compilation.start_debugging (!(Private.main_ref));;
let start_executing short_path= Mw_with_batch_compilation.start_executing (!(Private.main_ref)) short_path;;
...
let sugared_above capitalized_or_not_module_name=
  ...
  (Mw_with_dependencies.ancestors_for_module (!(Private.main_ref)) mn0);;

let sugared_below capitalized_or_not_module_name=
  let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
  Image.image Dfa_module.to_line
  (Mw_with_dependencies.below (!(Private.main_ref)) mn0);;

let sugared_directly_above capitalized_or_not_module_name=
  let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
  Image.image Dfa_module.to_line
  (Mw_with_dependencies.direct_fathers_for_module (!(Private.main_ref)) mn0);;

let sugared_directly_below capitalized_or_not_module_name=
let mn0 = Dfa_module.of_line(String.uncapitalize_ascii capitalized_or_not_module_name) in
Image.image Dfa_module.to_line
(Mw_with_dependencies.directly_below (!(Private.main_ref)) mn0);;
