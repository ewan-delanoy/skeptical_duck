...
#use"lib/Mad_world/Filewatching/mw_module_small_details.ml";;
...
let compute_details_from_acolytes_list_for_one_module l=
   ...
   let principal_coatoms = Mw_file_small_details.used_modules principal_details in 
   let (all_coatoms,opt_mli_mt) = (
      match opt_mli_detailed_rless with 
      None -> (principal_coatoms,None)
      |Some(_,mli_details) ->
         let mli_coatoms = Mw_file_small_details.used_modules mli_details in 
         (principal_coatoms @ mli_coatoms,
         Some(Mw_file_small_details.modification_time mli_details))    
   ) in
   let all_coatoms_in_order = Ordered.sort lex_order all_coatoms in 
   {
      Mw_module_small_details_t.used_modules = all_coatoms_in_order ;
      used_libraries = Mw_file_small_details.used_libraries principal_details ;
      has_printer = Mw_file_small_details.has_printer principal_details ;
      ...
      principal_modification_time = Mw_file_small_details.modification_time principal_details ;
...
let has_printer fw = fw.Mw_module_small_details_t.has_printer ;;  
...
let modularize_details fw  = 
   let u_files=Mw_with_small_details.usual_compilable_files fw in 
   let temp1=List.filter (fun (rl,_)->List.mem rl u_files)
      (Mw_poly.small_details_in_files fw)  in
...
let mli_present fw = fw.Mw_module_small_details_t.mli_present ;; 
let opt_mli_modification_time fw = fw.Mw_module_small_details_t.mli_modification_time ;;      
let principal_ending fw = fw.Mw_module_small_details_t.principal_ending ;; 
let principal_modification_time fw = fw.Mw_module_small_details_t.principal_modification_time ;;     
let recompute_details_for_module = Private.recompute_details_for_module ;; 
let subdirectory fw = fw.Mw_module_small_details_t.subdirectory ;;  
let used_libraries fw = fw.Mw_module_small_details_t.used_libraries ;;  
let used_modules fw = fw.Mw_module_small_details_t.used_modules ;;  
