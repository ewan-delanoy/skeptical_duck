...
#use"lib/Mad_world/Filewatching/mw_with_archives.ml";;
...
module Private = struct
   ...
   (* let configuration fw = Mw_file_watcher.configuration (parent fw) ;;*)

   let root fw = Mw_poly.root fw ;;
   let update_parent fw new_parent = Mw_poly.set_parent ~child:fw ~new_parent ;;
   let watched_files fw = Mw_poly.watched_files fw ;;
   ...
   let constructor par opt_subdirs= 
      ...
      Mw_poly.extend_file_watcher_to_fw_with_archives
        par ~subdirs_for_archived_mlx_files:subdirs ;;
      
   let plunge_fw_configuration config = constructor(Mw_file_watcher.plunge_fw_configuration config) None ;;
   let of_configuration config = constructor(Mw_file_watcher.of_configuration config) None ;;
   let of_configuration_and_list config l = constructor (Mw_file_watcher.of_configuration_and_list config l) None;;
   let overwrite_file_if_it_exists fw rl new_content = 
      let old_parent = parent fw in 
      let (new_parent,file_exists) = Mw_file_watcher.overwrite_file_if_it_exists old_parent rl new_content in 
      (update_parent fw new_parent,file_exists);;
   let register_rootless_paths fw rls = 
      let old_parent = parent fw in 
      let new_parent = Mw_file_watcher.register_rootless_paths old_parent rls in 
      update_parent fw new_parent ;;   
   let remove_files fw rls = 
      let old_parent = parent fw in 
      let new_parent = Mw_file_watcher.remove_files old_parent rls in 
      update_parent fw new_parent ;;               
   let rename_subdirectory_as fw sd_pair = 
      let old_parent = parent fw in 
      let (new_parent,extra) = Mw_file_watcher.rename_subdirectory_as old_parent sd_pair in 
   ...
   let canonical_tripartition fw all_files =
      ...
      let archived_subdirs = Mw_poly.subdirs_for_archived_mlx_files fw in 
   ...
   let forget_modules fw mod_names =
      ...
      let old_parent = parent fw in    
      let new_parent = Mw_file_watcher.remove_files old_parent the_files in 
   ...
   let inspect_and_update old_fw = 
      ...
      let (new_parent,changed_files) = 
          Mw_file_watcher.inspect_and_update old_parent
           ~verbose:false in 
   ...
   let latest_changes fw = 
      let changed_files = Mw_file_watcher.latest_changes (parent fw) ~verbose:false in 
   ...
   let relocate_module_to fw mod_name new_subdir=
      ...
      let new_parent = Mw_file_watcher.rename_files old_parent replacements in 
   ...
   let replace_value old_fw (preceding_files,path) (replacee,pre_replacer) =
      ...
      let rootless = Dfn_common.decompose_absolute_path_using_root path 
        (Mw_poly.root old_parent)  in 
      let par2= Mw_file_watcher.update_some_files old_parent [rootless] in 
   ...
   let check_that_no_change_has_occurred fw =
      Mw_file_watcher.check_that_no_change_has_occurred (parent fw) ;; 
