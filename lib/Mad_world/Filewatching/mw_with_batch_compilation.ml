...
#use"lib/Mad_world/Filewatching/mw_with_batch_compilation.ml";;
...
module Private = struct 
  ...
  let get_cmpl_results fw = Mw_poly.last_compilation_result_for_module fw ;;
  let set_cmpl_results fw new_list = 
    Mw_poly.set_last_compilation_result_for_module fw new_list ;; 
  let set_parent fw new_parent = 
      Mw_poly.set_parent ~child:fw ~new_parent:new_parent ;;
  let usual_extension fw cmpl_results =
      Mw_poly.extend_fw_with_dependencies_to_fw_with_batch_compilation    
  ...
  let modules_with_their_ancestors fw l=
    Mw_with_dependencies.modules_with_their_ancestors
     (parent fw) l ;;
  ...
  let preq_types_with_extra_info fw =
      ...
      ) (Mw_with_dependencies.printer_equipped_types fw) ;;  
  
  
  module Command = struct 
  
      let module_separate_compilation cmod fw eless pr_ending=
       Mw_commands_for_batch_compilation.module_separate_compilation 
         cmod (parent fw) eless pr_ending;;
      
      let predebuggable fw short_path =
        Mw_commands_for_batch_compilation.predebuggable 
          (parent fw) short_path ;; 
      
      let debuggable_or_executable cmod fw rootless_path =
        Mw_commands_for_batch_compilation.debuggable_or_executable
  ...
  let rec helper_for_feydeau  (cmod:Compilation_mode_t.t) fw (rejected,treated,to_be_treated)=
       ...
       |triple::other_triples->
         ...
         then let fw2= set_cmpl_result_at_module fw nm true in 
             ...
             (
                fun (nm2,_,_)->
                  List.mem nm (Mw_with_dependencies.ancestors_for_module fw nm2)
             ) triples_after in 
  ...
  let prepare_pretty_printers_for_ocamldebug fw deps = 
    ...
    let printable_deps = List.filter (
      fun mn -> let eless = Mw_with_dependencies.endingless_at_module fw mn in 
  ...
  let dependencies_inside_shaft cmod fw (opt_modnames,opt_rootless_path)=
     ...
     |Compilation_mode_t.Executable->let rootless_path=Option.get opt_rootless_path in 
         ...
         let deps =List.filter (fun mn->List.mem mn nm_deps) (Mw_with_dependencies.dep_ordered_modules fw) in 
  ...
  let list_of_commands_for_shaft_part_of_feydeau cmod fw (opt_modulenames,opt_rootless_path)=
     ...
     let temp1=Image.image (fun mn->
       let eless=Mw_with_dependencies.endingless_at_module fw mn 
       and pr_ending = Mw_with_dependencies.principal_ending_for_module fw mn in 
       let cmds=Command.module_separate_compilation cmod fw eless pr_ending in 
      Image.image (fun cmd->(mn,Mw_with_dependencies.endingless_at_module fw mn,cmd) ) cmds ) l in 
      List.flatten temp1;;
  ...
  let list_values_from_module fw mn = Mw_with_dependencies.list_values_from_module (parent fw) mn ;;
  let show_value_occurrences fw mn = Mw_with_dependencies.show_value_occurrences (parent fw) mn ;;

  let up_to_date_elesses fw =
    List.filter_map (
      fun mn->
        if last_compilation_result_for_module fw mn
        then Some(Mw_with_dependencies.endingless_at_module fw mn)
        else None
    )(Mw_with_dependencies.dep_ordered_modules fw);;

  let number_of_modules fw = Mw_with_dependencies.number_of_modules (parent fw) ;;  
  ...
  let modern_recompile fw changed_modules_in_any_order = 
      ...
      let (all_deps,new_deps,_changed_modules) = 
        Mw_with_dependencies.below_several (parent fw) changed_modules_in_any_order in     
   ...
   let forget_modules fw mod_names=
      let (new_parent,removed_files) = Mw_with_dependencies.forget_modules (parent fw) mod_names in  
   ...
   let remove_files fw rootless_paths=
      let (new_parent,_)=Mw_with_dependencies.remove_files (parent fw) rootless_paths in   
   ...
   let inspect_and_update fw =
      let (new_parent,((_changed_archived_compilables,changed_usual_compilables),_,changed_files))
         =Mw_with_dependencies.inspect_and_update (parent fw) in   
   ...
   let register_rootless_paths fw rps=
      let (new_parent,((_ac_paths,uc_paths,_nc_paths),_))=
       Mw_with_dependencies.register_rootless_paths (parent fw) rps in   
      let old_list_of_cmpl_results= get_cmpl_results fw in 
     let new_list_of_cmpl_results = Image.image (
        fun mn -> 
     ...
     ) (Mw_with_dependencies.dep_ordered_modules new_parent) in 
   ...
   let relocate_module_to fw mod_name new_subdir=
      let (new_parent,replacements)=Mw_with_dependencies.relocate_module_to (parent fw) (mod_name,new_subdir) in   
   ...
   let rename_module fw old_middle_name new_nonslashed_name=
     ...
     let separated_acolytes_below=List.filter_map(
       fun mn->
        if List.mem old_nm (Mw_with_dependencies.ancestors_for_module old_parent mn)
       then Some(Image.image (Dfn_full.to_rootless) (Mw_with_dependencies.acolytes_at_module old_parent mn))
       else None
   ) (Mw_with_dependencies.dep_ordered_modules old_parent) in
     let all_acolytes_below=List.flatten separated_acolytes_below in
     let (new_parent,changes) = Mw_with_dependencies.rename_module_on_filename_level_and_in_files 
   ...
   let rename_subdirectory_as fw (old_subdir,new_subdir)=
      let (new_parent,extra)=Mw_with_dependencies.rename_subdirectory_as 
         (parent fw) (old_subdir,new_subdir) in   
         (set_parent fw new_parent,extra) ;;
   
    let replace_string fw old_s new_s =
         let old_parent = parent fw in 
         let (new_parent,(changes1,all_changed_files)) = Mw_with_dependencies.replace_string old_parent (old_s,new_s) in 
    ...
    let replace_value fw ((preceding_files,path),(old_v,new_v)) =
          let old_parent = parent fw in 
          let (new_parent,(u_changes,all_changes)) = Mw_with_dependencies.replace_value old_parent ((preceding_files,path),(old_v,new_v)) in 
  ...
  let plunge_fw_configuration config = 
    usual_extension (Mw_with_dependencies.plunge_fw_configuration config) [];;
