...
module Private = struct
 ...
 let expand_index idx = (idx,Mw_indexer.get_state idx) ;;
 let index fw = Mw_poly.index_for_caching fw ;; 
 let parent fw = Mw_poly.parent fw ;;
 let usual_extension fw_parent instance_idx = 
    Mw_poly.extend_fw_with_small_details_to_fw_with_dependencies 
      fw_parent 
...
let forget_modules = Mw_with_small_details.forget_modules ;;

let inspect_and_update = Mw_with_small_details.inspect_and_update ;;

let of_configuration = Mw_with_small_details.of_configuration ;;

let of_configuration_and_list = Mw_with_small_details.of_configuration_and_list ;;

let overwrite_file_if_it_exists = Mw_with_small_details.overwrite_file_if_it_exists ;;

let plunge_fw_configuration = Mw_with_small_details.plunge_fw_configuration ;;

let register_rootless_paths = Mw_with_small_details.register_rootless_paths ;;

let relocate_module_to = Mw_with_small_details.relocate_module_to ;;

let remove_files = Mw_with_small_details.remove_files ;;

let rename_module_on_filename_level_and_in_files = Mw_with_small_details.rename_module_on_filename_level_and_in_files ;;

let rename_subdirectory_as = Mw_with_small_details.rename_subdirectory_as ;;

let replace_string = Mw_with_small_details.replace_string ;;

let replace_value = Mw_with_small_details.replace_value ;;end ;;
...
let forget_modules old_fw mods_to_be_erased =  
 ...
 let _ = Mw_indexer.push_state instance_idx in 
...
let of_configuration config =  
 ...
 let instance_idx = Mw_indexer.create_new_instance () in 
...
module Modularized_details = struct 
 ...
 let force_get fw = 
  ...
  let u_files=Mw_with_small_details.usual_compilable_files par_fw 
  and small_details = Mw_poly.small_details_in_files par_fw in 
  Mw_module_small_details.modularize_from_compilable_files_and_small_details u_files small_details ;;
 let get fw = 
   let idx = index fw in 
...
let inspect_and_update old_fw  =  
 ...
 if temp1 <> []
 then (mn, Mw_module_small_details.compute_details_from_acolytes_list_for_one_module temp1)
    else old_pair
 ) in 
 ...
 let tempf = (
        fun old_pair -> 
          ...
          then let new_parent = parent new_fw in 
               (mn, Mw_module_small_details.recompute_details_for_module (Mw_with_small_details.small_details_in_files new_parent)
                    mn temp1)
          else old_pair 
      ) in 
 ...
 let (new_fw,extra) = visible in 
  ...
  let tempf1 = (
    fun old_pair -> 
      ...
      if temp1 <> []
      then let new_parent = parent new_fw in 
           (mn, Mw_module_small_details.recompute_details_for_module (Mw_with_small_details.small_details_in_files new_parent) mn temp1)
      else old_pair 
  ) in 
  let answer = (Image.image tempf1 old_val)@
  (Mw_module_small_details.compute_details_from_acolytes_list_for_several_modules nonoverlapping) in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 visible ;;
...
module Order = struct 
 ...
 let the_hashtbl = ((Hashtbl.create 10)) ;; 
 let force_get fw = Mw_determine_order.main (Modularized_details.get fw)
 let get fw = 
   let idx = index fw in 
   match Hashtbl.find_opt the_hashtbl idx with 
      Some(old_answer)-> old_answer 
...
let inspect_and_update old_fw  =  
 ...
 let answer = Mw_determine_order.main  details_in_old_order in 
 let _ = Hashtbl.add the_hashtbl (index new_fw) answer in 
 visible ;;
...
let register_rootless_paths old_fw rootlesses =  
 ...
 let new_modules_in_order = Image.image fst (Mw_determine_order.main new_details) in 
 let new_details_in_order = Ordered_misc.reorder_list_of_pairs_using_list_of_singles
     new_details new_modules_in_order in 
 let answer = Mw_determine_order.compute_coatoms_and_ancestors_in_small_extension
...
module Needed_dirs = struct 
 ...
 let subdir_at_module = (fun mn->
   Mw_module_small_details.subdirectory(List.assoc mn details)
 ) in 
 Image.image (
  fun (mn,(_,ancestors)) ->
...
module Needed_libs = struct 
 ...
 let needed_libs_at_module = (fun mn->
   Mw_module_small_details.used_libraries(List.assoc mn details)
 ) in 
 Image.image (
  fun (mn,(_,ancestors)) ->
...
module All_subdirectories = struct 
 ...
 Ordered.sort Total_ordering.standard (Image.image (
  fun (_,details_on_mn) ->
  Mw_module_small_details.subdirectory(details_on_mn)
) details) ;;
 let get fw = 
   let idx = index fw in 
   match Hashtbl.find_opt the_hashtbl idx with 
...
module All_printables = struct 
 ...
 let force_get fw =  let mods_without_subdirs = List.filter_map (
  fun (mn,details) ->
 if Mw_module_small_details.has_printer details
  then Some mn
  else None
 ...
 Image.image (
    fun mn ->
      ...
      let subdir = Mw_module_small_details.subdirectory details in 
...
(* Pre-processed text ends here *)
  ...
  let check_ending_on_module fw edg  mn=
   if edg=Mw_module_small_details.principal_ending (details_for_module fw mn)
   then true 
   else 
   if edg=Dfa_ocaml_ending_t.Mli
   then Mw_module_small_details.mli_present (details_for_module fw mn) 
   else false;;
  ...
  let root fw = Mw_poly.root  (parent fw) ;;

  let subdir_for_module fw mn =Mw_module_small_details.subdirectory (details_for_module fw mn) ;;
...
) (Mw_with_archives.archived_files cs);;
...
let decipher_module fw capitalized_or_not_x=
  ...
  let modules_using_value fw value_name =
    List.filter_map (fun (mn,_)->
      ...
      and pr_end=Mw_module_small_details.principal_ending (details_for_module fw mn) in
...
let those_above=(snd (List.assoc nm (Order.get fw)))@[nm] in
let temp1=Image.image (
    fun nm2->
      let hm2= endingless_at_module fw nm2 in
...
let of_concrete_object crobj = 
    let instance_idx = Mw_indexer.create_new_instance () in   
    Mw_poly.extend_fw_with_small_details_to_fw_with_dependencies 
      (Mw_poly.of_concrete_object crobj) 
...
let principal_ending_for_module fw mn = Mw_module_small_details.principal_ending (Private.details_for_module fw mn) ;;
...
let root fw = Mw_poly.root (Private.parent fw) ;;

let self_contained_module_copy = Private.self_contained_module_copy ;;
let show_value_occurrences = Private.show_value_occurrences ;;
let subdir_for_module fw mn = Mw_module_small_details.subdirectory (Private.details_for_module fw mn) ;;
let to_concrete_object fw = Mw_poly.to_concrete_object (Private.parent fw) ;;
let usual_compilable_files fw = Mw_with_small_details.usual_compilable_files (Private.parent fw) ;;
