...
#use"lib/Mad_world/mw_check_ocaml_dircopy.ml";;
...
let filter_diff_according_to_admissibility data diff=
   let filter_list = List.filter (Mw_configuration.test_for_admissibility data) in 
   {
...
let commands_for_confidentiality encoding_protected_files =
   Image.image (
     fun (replacer,replacee) ->
       ...
       let s_full_path = Mw_constant.clone_download_location^"/"^(Dfn_rootless.to_line replacee) in 
...
let check fw_data github_data =
  let name_of_clone_directory = Mw_constant.clone_download_location in 
  let i=(
  ...
  let full_clone_command=
    "git clone "^
    (Mw_poly.github_url github_data)^" "^
    name_of_clone_directory in 
  let j=Unix_command.uc full_clone_command in
  if j<>0
  then raise(Failure_during_github_cloning)
  else 
  let cmds = commands_for_confidentiality (Mw_poly.encoding_protected_files github_data) in 
  let _= Unix_command.conditional_multiple_uc cmds in 
  let root_dir = Mw_poly.root github_data in 
  let diff1=Prepare_dircopy_update.compute_restricted_diff
     root_dir remotedir (Mw_poly.ignored_subdirectories fw_data,
        (Image.image Dfn_rootless.to_line (Mw_poly.ignored_files fw_data) )) in
