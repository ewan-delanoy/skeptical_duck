(*
#use"lib/Mad_world/mw_self_contained_module_copy.ml";;
*)
...
let self_contained_module_copy prefix hm=
   let cs=(!(Mw_usual_coma_state.main_ref)) in 
   let nm=Dfn_endingless.to_module hm in
   let those_above=(Mw_with_dependencies.ancestors_for_module cs nm)@[nm] in
   let temp1=Image.image (
       fun nm2->
         let hm2=Mw_with_dependencies.endingless_at_module cs nm2 in
