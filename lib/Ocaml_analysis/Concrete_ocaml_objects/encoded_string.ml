...
let decode (Encoded_string_t.E(encoded_s))=
   Replace_inside.replace_inside_text ~display_number_of_matches:false (replacement_salt,salt) encoded_s;;

let encode s=
   if Substring.is_a_substring_of replacement_salt s 
   then raise(Forbidden_substring)
   else let encoded_s=Replace_inside.replace_inside_text ~display_number_of_matches:false (salt,replacement_salt) s in 
