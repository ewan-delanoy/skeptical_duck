...
let to_list (M l)=Image.vorstellung(fst)(l);;

let filter_odd_multiplicities (M l)=
   let temp1=List.filter(function (x,j)->(j mod 2)=1)(l) in
   Image.vorstellung(fst)(temp1);;
            
let length (M l)=Basic.fold_sum(Image.vorstellung snd l);;            
...
let diforchan x=
ddiforchan(Image.vorstellung(function u->(u,1))(x));;
