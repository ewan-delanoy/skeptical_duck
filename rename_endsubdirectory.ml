...
let in_unix_world root_dir (old_subdir,new_esdname)=
   ...
   let new_name=s_root^(Cull_string.before_rightmost s_old_subdir '/')^"/"^new_esdname in
   if Sys.file_exists(new_name)
   then raise(Already_present_directory(new_name))
   else 
   let container=Cull_string.before_rightmost  new_name '/' in
   let _=
   Unix_command.uc 
...
let re (old_subdir,new_esdname) s=
   let s_old_subdir=Dfa_subdirectory.without_trailing_slash old_subdir in
   if Supstring.begins_with s s_old_subdir
   then let sub_s=Cull_string.cobeginning (String.length s_old_subdir) s in
        (Cull_string.before_rightmost s_old_subdir '/')^"/"^new_esdname^sub_s
   else s;;
   
let on_absolute_path root_dir (old_subdir,new_subdirname) ap=
  let s_old_subdir=Dfa_subdirectory.connectable_to_subpath old_subdir in
  let s_ap=Absolute_path.to_string ap in
  let old_fulldir=(Dfa_root.connectable_to_subpath root_dir)^s_old_subdir in
  if Supstring.begins_with s_ap old_fulldir
