...
let self_contained_module_copy prefix hm=
   ...
   let replacements=Image.image (
       fun ((a,b),_)->(Dfa_module.of_string a,Dfa_module.of_string b)
   ) temp1 in
   ...
   let temp3=Image.image (
     fun ((_,new_mname),txt)->
      let name=String.capitalize_ascii(Dfa_module.to_string new_mname) in
