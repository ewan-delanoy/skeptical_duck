...
let pusher_for_hierarchization (graet,hollad,da_ober)=
  ...
  let temp1=Image.imagination (fun (x,y)->(x,Set_of_polys.setminus y hollad)) da_ober in
  let (temp2,temp3)=List.partition (fun (x,z)->Set_of_polys.length z=0) temp1 in
  if temp2=[]
  then Failed(graet,hollad,da_ober)
  else
  let temp4=Image.imagination fst temp2 in
  let o_temp4=Set_of_polys.sort(temp4) in
  let temp5=Image.imagination (fun (x,z)->(x,Set_of_polys.setminus z o_temp4)) temp3 in
...
let try_hierarchizing (f: 'a list_of_ancestors_map) l=
  let temp1=Image.imagination (fun t->(t,Set_of_polys.sort(f t))) l in
  let rec tempf=(fun x->
