...
let rec morzholan f (an_holl,da_ober)=
  if da_ober=Set_of_polys.empty_set
  then an_holl
...
let one_more_time2 f (an_holl,graet,da_ober)=
 ...
 and l_da_ober=Set_of_polys.forget_order(da_ober) in
...
type 'a hierarchize_data=
   Normal of  'a Set_of_polys_t.t list * 'a Set_of_polys_t.t * ('a * 'a Set_of_polys_t.t) list
  |Failed of  'a Set_of_polys_t.t list * 'a Set_of_polys_t.t * ('a * 'a Set_of_polys_t.t) list
...
let pusher_for_hierarchization (graet,hollad,da_ober)=
  ...
  let temp1=Image.vorstellung (fun (x,y)->(x,Set_of_polys.setminus y hollad)) da_ober in
  let (temp2,temp3)=List.partition (fun (x,z)->Set_of_polys.length z=0) temp1 in
  if temp2=[]
  then Failed(graet,hollad,da_ober)
  else
  let temp4=Image.vorstellung fst temp2 in
  let o_temp4=Set_of_polys.sort(temp4) in
  let temp5=Image.vorstellung (fun (x,z)->(x,Set_of_polys.setminus z o_temp4)) temp3 in
...
let try_hierarchizing (f: 'a list_of_ancestors_map) l=
  let temp1=Image.vorstellung (fun t->(t,Set_of_polys.sort(f t))) l in
  let rec tempf=(fun x->
