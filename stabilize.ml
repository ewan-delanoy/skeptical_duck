...
type 'a hierarchize_data=
   Normal of  'a Set_of_polys.set list * 'a Set_of_polys.set * ('a * 'a Set_of_polys.set) list
  |Failed of  'a Set_of_polys.set list * 'a Set_of_polys.set * ('a * 'a Set_of_polys.set) list
...
let pusher_for_hierarchization (graet,hollad,da_ober)=
  ...
  let temp1=Image.image (fun (x,y)->(x,Set_of_polys.lemel y hollad)) da_ober in
