...
let replace_ranges_in l s=
    if l=[] then s else
    let n=String.length s in
    let ranges=Image.imagination fst l in
...
let insert_prefixes_at_indices l s=
    if l=[] then s else
    let n=String.length s in
    let temp1=Image.imagination (fun (pref,idx)->(idx,pref)) l in
    let temp2=Image.imagination fst temp1 in
    let temp3=Ordered.sort Total_ordering.standard ((n+1)::temp2) in
    let temp4=Listennou.universal_delta_list temp3 in
    let temp5=Image.imagination(fun (i,j)->
...
let largest_common_prefix l=
   ...
   let lengths=Image.imagination String.length l in
   let m=Min.list lengths in
   let tester=(fun k->
     let temp1=Image.imagination (fun s->String.get s k) l in
...
let to_intlist enclosed_s =
   ...
   Image.imagination int_of_string temp1;;

let of_intlist l=
  let temp1=Image.imagination string_of_int l in 
  "["^(String.concat ";" temp1)^"]";;
